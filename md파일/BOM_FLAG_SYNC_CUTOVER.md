...
- **검증**: 재편성 후 4930·5210 인입 ✓. **표본 120품목 완전일치 118·근접±1 2·불일치 0**(RAC 용접봉=proc_weld 설계차이 제외). AJR75563402 = 8/9(레거시만 RAC뿐).
- ★gap②(사급자식)는 별도 사급로직이 아니라 **5210도 910이었던 것** — 910 제거로 함께 해결.
- 남은 유일 차이 = **RAC 용접봉**(우리 proc_weld 설계, 의도적). 컷오버 대사 시 이 차이만 인지.

## 6. (구) 잔여 갭 분석 — ↑6-1에서 해결됨. 아래는 규명 이력.
