...
│   └── claude-main (codex 미구독으로 흡수 배정 — 이미지 생성은 대체 워커 없어 불가)
│
├── [computer-use] 브라우저 조작 · 복잡한 도구 워크플로우 자동화?
│   └── (대체 워커 없음 — codex 미구독으로 현재 미사용)
│
├── [reviewer] 산출물 리뷰 / 비판적 검증?
│   └── gemini (codex-critic 대체 — codex 미구독. 벤더 다른 gemini로 자기검수 회피 유지)
│
...
> codex(codex-main·codex-critic)는 기본 미구독 가정으로 미사용. 위 배정은 `capability-profile.md`
> 2026-08-13 이력 항목의 병기 사본 — codex 구독 시 프로필부터 갱신 후 이 트리를 되돌린다.

## 복합 작업 우선순위

한 작업이 여러 분기에 해당할 때:

1. **선행 의존성 우선**: reviewer(gemini)는 리뷰 대상(보통 claude-main 결과)이 먼저 있어야 함 → 해당 산출물 뒤에 호출
...
3. **검증은 한 번만**: reviewer(gemini)는 작업당 1회 원칙. 재호출은 검증 실패 시만
...
| 패턴 | 언제 | 이 시스템에서 |
|------|------|-------------|
| Pipeline (순차) | 앞 결과가 뒤 입력 | 기본. claude-main → gemini(리뷰) → claude-main(반영) |
...
| Producer-Reviewer (생성+게이트) | 산출물 품질 검증 필요 | claude-main(생성) → gemini(adversarial 게이트, codex-critic 대체) |
...
### codex-main (기본 미사용 — codex 미구독 가정)
...
### codex-critic (기본 미사용 — codex 미구독 가정. reviewer 슬롯은 gemini가 대체 — 아래 gemini 항목 참조)
- **슬롯**: reviewer
...
### gemini
- **슬롯**: multimodal · reviewer (codex-critic 대체, codex 미구독 가정)
- **용도**: 이미지/스크린샷/다이어그램 분석, 50페이지+ 문서 스캔, 제3자 시각의 검토, 산출물 리뷰·비평(reviewer 슬롯 — claude-main 산출물을 벤더 다른 시각에서 검증)
- **결과물**: 분석 텍스트, 요약, (reviewer 역할 시) 비평 리스트·수정 제안
- **호출 명령**: `_shared/backends.json`의 `gemini` 항목이 정본. 디스패처로 호출:
  ...
  ```
  백엔드 = Antigravity `agy` CLI(헤드리스), 기본 `gemini-3.6-flash-high`, 폴백 없음(아래 폴백 조건). 폐기: `mcp__gemini-pro__*`·`mcp__gemini__*` 프록시 브리지.
...
- **codex-main / codex-critic** (기본 미사용 — codex 미구독 가정): 구독 시 사용자의 `~/.codex/config.toml` 기본값이 자동 적용된다. config.toml이 정본이라 여기에 버전을 핀하지 않는다. MCP 호출 시 `model` 파라미터를 비워두면 config 기본값 사용.
  - 가벼운 작업은 `profile: lightweight`로 전환 가능 (config.toml의 가벼운 모델 프로필)
  - 작업 성격상 다른 모델이 필요하면 brief.md에 명시
- **gemini**: 백엔드 = Antigravity **`agy` CLI**(`_shared/backends.json` 정본, 디스패처 `call_worker.sh`). 기본 `gemini-3.6-flash-high`(reviewer 슬롯 겸용으로 빠른 경로를 기본화, `agy models`로 확인한 실제 모델 ID), pro 계열 필요 시 backends.json `model`을 `gemini-3.1-pro-high` 등으로 조정, 폴백 없음(api 슬롯 미구현·비활성, D11). 옛 `mcp__gemini-pro__*` 프록시 브리지·CLI 래퍼 `mcp__gemini__*`는 **폐기**. agy 모델은 전역·계정단위(`/model`)라 per-call 핀 불가 → gemini 전용 전역을 backends.json이 정한 값으로 둔다. 근거: `_shared/learnings.md` [2026-06-02] · `design-basis.md` D4.
...
| 대규모 구현·테스트 | claude-main (설계·구현·테스트 일괄 — codex-main 미사용) |
| 브라우저 자동화 / 이미지 생성 | 불가 (대체 워커 없음 — codex 미구독 가정) |
| 구현 + 비평 | claude-main(생성) → gemini(비평) → claude-main(반영) |
| 대용량 문서 처리 | gemini |
| 전체 검토 | claude-main → gemini |
