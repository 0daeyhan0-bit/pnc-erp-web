...
- **★글자 앞 장식 아이콘 금지 — 단, 기능 아이콘은 보존(전 프로그램).** **텍스트 바로 앞에 붙은 쓸데없는 장식 아이콘/이모지만 제거**(🔍💲🔄🎯⬇📅🔗🧾 등): "🔍 조회"→"조회", "🎯 회수율"→"회수율", "⬇ 엑셀"→"엑셀". **★단독으로 기능하는 아이콘 버튼(텍스트 없이 아이콘 자체가 버튼: 🗑삭제·✎편집·➕추가·⬇다운로드 등)은 절대 건드리지 말 것**(지우면 버튼이 빈칸=고장). 판별=아이콘 뒤에 **글자가 오면 장식(제거)**, **글자 없이 태그/따옴표가 오면 기능버튼(보존)**. 일괄 치환 시 `아이콘+공백`을 무조건 지우면 기능버튼까지 깨짐 — 반드시 "뒤에 글자 있는 것만" 제거. 신규 화면은 처음부터 장식 아이콘 없이. (상태/의미 전달용 ⚠경고·▲▼정렬·└트리·🔒권한 등도 보존.)
