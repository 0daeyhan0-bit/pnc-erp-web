...
- **★★★모든 문서·기록은 내 PC + 서버(Gitea main)에 반드시 올린다 (전 개발자·예외 없음, 대표 확정 2026-09-07).**
  - **대상 = 코드뿐 아니라 "모든 기록"**: durable 노트, 정본(canon)·규칙 문서, 설계(design) 문서, 검증·대사·실행 로그, 인벤토리, 마이그레이션 기록, 의사결정 경위 등 `_schema/`·`_docs/` 이하 전부.
  - **왜**: Gitea(184) main = 다른 개발자가 실제로 보는 공유 저장소. 로컬(내 PC)이나 feature 브랜치에만 있으면 **다른 개발자에게 안 보인다** → 같은 함정·재삽질·중복작업이 반복된다.
  - **어떻게**: 문서를 신규작성·수정하면 (a) 내 PC 로컬 repo에 커밋 → (b) **Gitea로 push → PR → main 병합**. 코드와 함께 나온 기록은 같은 PR에, 문서만이면 `docs/<주제>` 브랜치로 문서-only PR. **feature 브랜치에만 두고 방치 금지**(main에 올려야 공유됨).
  - **언제**: 프로그램을 수정하거나 신규개발할 때마다 그 기록을 함께 올린다("나중에 몰아서" 금지 — 작업과 같은 흐름에서 즉시).
  - **정본 갱신 연쇄**: 새 정본·충돌·완료가 생기면 `_schema/00_MASTER_INDEX.md`부터 갱신(§상단 규칙)하고 그 변경도 main에 push.
