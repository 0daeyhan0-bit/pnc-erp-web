...
- **★배포 후 확인 = 파일이 아니라 `openapi.json`(2026-08-31 실측 사고)**. `git pull` 이 성공해도
  **백엔드 프로세스가 재기동되지 않으면 구 코드가 계속 돈다.** 정적파일(JS)은 요청 시 디스크에서 읽으므로
  **프론트만 최신이 되고 백엔드는 옛것**인 어긋난 상태가 된다 — 겉으로는 배포된 것처럼 보인다.
  실측: 운영 프론트 = 16:57 이후 파일인데 `/api/lgbom/upload` 에 `werks` 파라미터가 없었다(16:18 커밋 미반영).
  화면은 공장 선택을 보내는데 백엔드가 그 값을 모르는 상태였다.
  ⟹ 배포 후 **반드시** `curl -s http://<서버>:8010/openapi.json` 로 이번에 바뀐 시그니처가 실제로 떴는지 확인.
