...
-- =============================================
CREATE PROCEDURE [dbo].[SP_CS_견적서(실원가용)_250910]
...
/* 
	begin
		exec dbo.[SP_CS_견적서(실원가용)] 'AJR30001402', '241105'
	end

	DECLARE	@AS_ITEM_CODE		VARCHAR(20)
	DECLARE	@AS_COST_APPLY_YMD		VARCHAR(20)

	SET @AS_ITEM_CODE = 'AJR30073603'
	SET @AS_COST_APPLY_YMD = CONVERT(VARCHAR, GETDATE(), 12)
*/
	...
	BEGIN
 		WITH CTE_BOM(assy_item_code, item_code, db_item_code, work_code, in_cust_code, level_num, bom_seq, mat_code, mat_work_code, mat_in_cust_code, parent_in_cust_code, use_qty, upper_use_qty, cum_use_qty, cum_mat_code,
							item_desc, item_spec, item_class, item_lgroup, item_sgroup, item_diam, item_thick, item_length, metal_gubun, pipe_kind, item_weight, unit, gravity, cost_gubun, VIR_ITEM_FLAG, INNER_PROD_FLAG, LME_CALC_FLAG, lme_except_flag)
		AS (
			/*앵커멤버-생산계획*/
				select
						...
						'0' AS VIR_ITEM_FLAG,
						IIF(ISNULL(C.MAKE_TYPE,'')='', IIF(C.IN_CUST_CODE = '' OR EXISTS(SELECT 1 FROM PR_M_ITEM_PROC_GAGONG WHERE ITEM_CODE = C.ITEM_CODE),'1','0')
															  , IIF(C.MAKE_TYPE='1','1','0')) AS INNER_PROD_FLAG,		/*사내생산여부*/
						'0' AS LME_CALC_FLAG,
				...
				select cb.assy_item_code,
						...
						IIF(CB.VIR_ITEM_FLAG='1', CB.parent_in_cust_code, IIF(CB.MAT_IN_CUST_CODE>'', CB.MAT_IN_CUST_CODE, '')) as parent_in_cust_code,
						...
						B.VIR_ITEM_FLAG,
						IIF(ISNULL(M.MAKE_TYPE,'')='', IIF(M.IN_CUST_CODE = '' OR EXISTS(SELECT 1 FROM PR_M_ITEM_PROC_GAGONG WHERE ITEM_CODE = M.ITEM_CODE),'1','0')
															  , IIF(M.MAKE_TYPE='1','1','0')) AS INNER_PROD_FLAG,		/*사내생산여부*/
						IIF(CB.LME_CALC_FLAG='1', '1', IIF(CB.INNER_PROD_FLAG='1','0','1')) AS LME_CALC_FLAG,
		...
		SELECT A.*
				...
				,convert(decimal(18,4),0) as won_mat_cost_sub		/*LME산출을 위한 LG절단재료비 - 협력사절단재료비*/
				,convert(decimal(18,4),0) as jai_cost
				,convert(decimal(18,4),0) as jai_cost_sub
	...
	UPDATE A
	  ...
	  FROM #TEMP_BOM A
	 WHERE LEVEL_NUM = 0
	   AND (SELECT COUNT(*) FROM #TEMP_BOM WHERE ASSY_ITEM_CODE = A.ASSY_ITEM_CODE AND LME_CALC_FLAG='0') > 1
	...
	/*LME계산을 위해서 구매부품 하위까지 BOM을 풀었기 때문에 단가구분이 있는 모든 부품에 단가를 세팅. 아래에서 다시 한번 계산함.*/
	UPDATE A
		SET JAI_COST = isnull(CASE COST_GUBUN WHEN '3' THEN WON_MAT_COST * ITEM_WEIGHT * USE_QTY ELSE WON_MAT_COST * USE_QTY END,0)
	  FROM #TEMP_BOM A
	 WHERE (COST_GUBUN > '' OR PARENT_IN_CUST_CODE>'' AND MAT_WORK_CODE = 'P2')

	/*LME계산은 최말단 부품에만 발생시킨다.*/
	UPDATE A
		SET JAI_COST_SUB = isnull(CASE WHEN ITEM_WEIGHT > 0 THEN WON_MAT_COST_SUB * ITEM_WEIGHT * USE_QTY ELSE 0 END,0)
	  FROM #TEMP_BOM A
	 WHERE (COST_GUBUN IN ('1','2') OR PARENT_IN_CUST_CODE>'' AND MAT_WORK_CODE = 'P2')
	   AND NOT EXISTS (SELECT 1 FROM #TEMP_BOM WHERE ITEM_CODE = A.MAT_CODE AND LEVEL_NUM = A.LEVEL_NUM + 1)
	...
	UPDATE A
		SET INNER_PROD_FLAG = IIF(M.SILVER_SOLDER>'','1',A.INNER_PROD_FLAG)
	  FROM #TEMP_BOM A
	  JOIN PR_M_ITEM M ON A.MAT_CODE = M.ITEM_CODE


	SELECT A.*
			,B.PROC_CODE, P.R_SORT_SEQ
			,IIF(A.INNER_PROD_FLAG='1',B.WORK_QTY,0) AS WORK_QTY
			,IIF(A.INNER_PROD_FLAG='1',B.PROD_UPH,0) AS PROD_UPH
			,IIF(A.INNER_PROD_FLAG='1',B.COST_GUBUN,'') AS CALC_GUBUN
			,IIF(A.INNER_PROD_FLAG='1',CONVERT(DECIMAL(18,4),CASE B.COST_GUBUN WHEN '3' THEN IIF(B.PROD_UPH * B.WORK_QTY=0, 0, ROUND(L.LABOR_COST / B.PROD_UPH * B.WORK_QTY, 0))			--임율
																									 WHEN '7' THEN 0																		--세척
																									 WHEN '8' THEN A.ITEM_WEIGHT * B.PROD_UPH * B.WORK_QTY	--중량
																									 WHEN '9' THEN B.PROD_UPH * B.WORK_QTY											--적용율 (일반관리비, 이윤 등)
																									 ELSE 0 END),0) AS PROD_AMT
	  INTO #TEMP_PROC_DTL
	  FROM #TEMP_BOM A
...
--	   AND B.COST_GUBUN = '9'
	...
	SELECT CUM_MAT_CODE
			...
			,MAX(MAT_WORK_CODE) AS MAT_WORK_CODE
			,'' AS WORK_CODE
			,0 AS SALE_COST
			...
			,0 AS LG_COST
			,MAX(LME_CALC_FLAG) AS LME_CALC_FLAG
	 ...
	 GROUP BY CUM_MAT_CODE, LEVEL_NUM
	...
	UPDATE A
	  ...
	  FROM #TEMP_BOM A
	 WHERE COST_GUBUN > ''
	   AND NOT EXISTS (SELECT 1 FROM #TEMP_BOM WHERE ITEM_CODE = A.MAT_CODE)
	*/
	...
	BEGIN
		...
		BEGIN
			...
			/*하위가 있으면 자신의 LME차액은 무시한다.*/
			UPDATE A
				SET LME_CHA_AMT = A.LME_CHA_AMT + (SELECT ISNULL(SUM(LME_CHA_AMT),0) FROM #TEMP_RESULT
																WHERE C_ITEM_LEVEL = @LI_LEVEL + 1
																  AND CUM_MAT_CODE LIKE A.CUM_MAT_CODE + '%' ) * IIF(UNIT='EA',USE_QTY,1)
			  FROM #TEMP_RESULT A
			 WHERE A.C_ITEM_LEVEL = @LI_LEVEL
			   AND (SELECT COUNT(*) FROM #TEMP_RESULT
										  WHERE C_ITEM_LEVEL = @LI_LEVEL + 1
											 AND CUM_MAT_CODE LIKE A.CUM_MAT_CODE + '%' ) > 0
		END

		--TRUNCATE TABLE CS_T_LME_RESULT

		--INSERT INTO CS_T_LME_RESULT
		--SELECT * FROM #TEMP_RESULT
		--SELECT * INTO CS_T_LME_RESULT
		--  FROM #TEMP_RESULT

		DELETE FROM #TEMP_RESULT
		 WHERE LME_CALC_FLAG = '1'
	END
	...
	/*재료비는 최말단 부품에만 발생시킨다.*/
	/*LME차이 금액은 재료비에 포함한다. 이여지과장 요청. 25/06/30*/
	UPDATE A
		SET JAI_COST = isnull(CASE B.COST_GUBUN WHEN '3' THEN B.WON_MAT_COST * B.ITEM_WEIGHT * B.USE_QTY ELSE B.WON_MAT_COST * B.USE_QTY END,0) + A.LME_CHA_AMT
			,BOTTOM_FLAG = '1'
	  FROM #TEMP_RESULT A
	  JOIN #TEMP_BOM B ON A.CUM_MAT_CODE = B.CUM_MAT_CODE
							AND A.C_ITEM_LEVEL = B.LEVEL_NUM
	 WHERE A.COST_GUBUN > ''
	   AND NOT EXISTS (SELECT 1 FROM #TEMP_RESULT WHERE ITEM_CODE = B.MAT_CODE AND C_ITEM_LEVEL = A.C_ITEM_LEVEL + 1)
	...
	/*일반관리비 계산 = (재료비 + 가공비 * 7%*/
	UPDATE #TEMP_RESULT
		SET ILBAN_AMT = ROUND(ILBAN_RATE * (JAI_COST - LME_CHA_AMT + GAGONG_AMT), 0)
	 WHERE ILBAN_RATE > 0
	...
	/*최종결과*/
	SELECT A.*
			,SUM(IIF(BOTTOM_FLAG='1' AND A.ITEM_SGROUP IN ('110','120','130','220'),JAI_COST,0) * UPPER_USE_QTY) OVER (PARTITION BY '')	AS WON_JAI_AMT		/*원자재 재료비*/
			,SUM(IIF(BOTTOM_FLAG='1' AND A.ITEM_SGROUP IN ('230','910'),JAI_COST,0) * UPPER_USE_QTY) OVER (PARTITION BY '')	AS BU_JAI_AMT		/*부자재 재료비*/
			,SUM(IIF(BOTTOM_FLAG='1' AND A.ITEM_SGROUP IN ('310'),JAI_COST,0) * UPPER_USE_QTY) OVER (PARTITION BY '')			AS SA_JAI_AMT		/*사급 재료비*/
			,M.SILVER_SOLDER
	  FROM #TEMP_RESULT A
