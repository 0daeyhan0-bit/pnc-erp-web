...
def main():
    ...
    res_lib = orca.PBORCA_SessionSetLibraryList(h_session, lib_arr, len(pbl_files))
    print("SessionSetLibraryList Result:", res_lib)
    ...
    for i, pbl_path in enumerate(sorted(pbl_files)):
        ...
        total_exported_all += success_count
        print("[{}/{}] {} -> Found: {}, Exported: {}".format(i+1, len(pbl_files), pbl_name, len(entries), success_count))

    orca.PBORCA_SessionClose(h_session)
    elapsed = time.time() - start_time
    print("\n==========================================")
    print("ALL PBL EXPORT V2 COMPLETED!")
    print("Total PBLs Processed: {}".format(len(pbl_files)))
    print("Total Objects Exported: {}".format(total_exported_all))
