...
def export_pbl(h_session, pbl_path, base_out_dir):
            ...
            for off in range(0, 1000, 2):
                ...
                if wstr and not wstr.startswith((' ', '\t', '\r', '\n')) and len(wstr) > 1:
                    if wstr.startswith(('w_', 'dw_', 'u_', 'm_', 'f_', 'd_', 'p_', 'cs_', 'pr_', 'sa_', 'pu_', 'hr_', 'ma_', 'qa_', 'cm_', 'app_', 'partnererp', 'ds_', 'tt_', 'mc_')):
                        ...
                        break
            if not found_name:
                found_name = raw_bytes[784:1024].decode('utf-16-le', errors='ignore').split('\x00')[0]
...
def main():
    ...
    for i, pbl_path in enumerate(sorted(pbl_files)):
        pbl_name = os.path.basename(pbl_path)
        try:
            succ, total = export_pbl(h_session, pbl_path, base_out_dir)
            total_objects += succ
    ...
    print("\n==========================================")
    print("100% PERFECT ALL PBL EXPORT COMPLETED!")
