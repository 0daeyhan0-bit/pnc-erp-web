...
print("Found {} entries in cm_com.pbl:".format(len(entries)))
...
for name, etype, esize, comment in entries:
    ...
    buf_len = 5 * 1024 * 1024 # 5MB buffer
    ...
    if res == 0:
        file_path = os.path.join(out_dir, "{}.{}".format(name, ext))
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(buf.value)
        success_count += 1
        print(" [OK] Exported: {} -> {}.{}".format(name, name, ext))
    else:
        print(" [FAIL] Code {}: {}, type {}".format(res, name, etype))

print("Export Summary: {}/{} succeeded.".format(success_count, len(entries)))
