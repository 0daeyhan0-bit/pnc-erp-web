ALTER PROCEDURE nx.SP_PR_가공창고_이동계획_WEBPLAN
	...
	SELECT * INTO #TEMP_PART_DTL
	  FROM (
				...
				SELECT  A.PLAN_YMD, A.WORK_ORDER, A.SPLIT_WORK_ORDER, A.C_ITEM_CODE, 0 BOM_LEVEL, A.C_ITEM_CODE, A.C_ITEM_CODE, 0 PROC_SEQ, 
						'', A.OUTPUT_HM, A.LINE_NO, 1 USE_QTY, A.PLAN_QTY * A.USE_QTY, '' WORK_CODE, '' GAGONG_PROC_CODE, 0 GAGONG_PROC_SEQ, '' JP_PROC_METHOD, 
				 ...
				 JOIN		nx.item M ON A.C_ITEM_CODE = M.ITEM_CODE
				WHERE		M.in_cust > ''
				...
				SELECT  A.PLAN_YMD, A.WORK_ORDER, A.WORK_ORDER, A.ITEM_CODE, 0 BOM_LEVEL, A.ITEM_CODE, A.ITEM_CODE, 0 PROC_SEQ, 
						'', A.OUTPUT_HM, A.LINE_NO, 1 USE_QTY, A.PLAN_QTY, '' WORK_CODE, '' GAGONG_PROC_CODE, 0 GAGONG_PROC_SEQ, '' JP_PROC_METHOD, 
						0 LT_HR, 0 CUM_LT_HR, A.PLAN_YMD AS PART_PLAN_YMD, A.OUTPUT_HM AS PART_OUTPUT_HM, 
				 ...
				 JOIN		nx.item M ON A.ITEM_CODE = M.ITEM_CODE
				WHERE		M.in_cust > ''
				  AND		A.PLAN_YMD >= CONVERT(VARCHAR, GETDATE(), 12)
				...
				SELECT  A.PLAN_YMD, A.WORK_ORDER, A.WORK_ORDER, A.ITEM_CODE, 0 BOM_LEVEL, A.ITEM_CODE, A.ITEM_CODE, 0 PROC_SEQ, 
						'', A.OUTPUT_HM, A.LINE_NO, 1 USE_QTY, A.PLAN_QTY, M.WORK_CODE, '' GAGONG_PROC_CODE, 0 GAGONG_PROC_SEQ, '' JP_PROC_METHOD, 
						0 LT_HR, 0 CUM_LT_HR, A.PLAN_YMD AS PART_PLAN_YMD, A.OUTPUT_HM AS PART_OUTPUT_HM, 
						A.PLAN_QTY AS PART_PLAN_QTY, 0 AS FINISH_TAG, 16777215 AS COLOR, 0 AS FINISH_QTY, 0 AS SALE_QTY, 
						0 AS ASSY_STOCK_QTY, 0 AS FIX_STOCK_QTY, 0 AS SET_STOCK_QTY, 0 AS PR_STOCK_QTY, 0 AS STOCK_QTY, 0 AS PART_STOCK_QTY, 
						0 AS READY_STOCK_QTY, 0 AS JP_PRINT_QTY, 0 AS READY_QTY, 0 CUM_JAN_QTY, A.ITEM_CODE AS CUM_ITEM_CODE, '' PRIOR_GAGONG_PROC_CODE, 0 PRIOR_GAGONG_PROC_SEQ
...
--	  order by t.ASSY_ITEM_CODE, t.BOM_LEVEL, t.UPPER_ITEM_CODE, t.ITEM_CODE, t.PROC_SEQ, t.PART_PLAN_YMD, t.PART_OUTPUT_HM, t.PLAN_YMD, t.OUTPUT_HM, t.WORK_ORDER, t.SPLIT_WORK_ORDER
			...
			select s.mat_code,
			  ...
			  from (
							...
							select a.mat_code, 0 AS stock_qty, A.STOCK_QTY AS PR_STOCK_QTY, 0 as SET_STOCK_QTY, 0 AS FIX_STOCK_QTY
								...
								join nx.item M WITH (NOLOCK) on A.MAT_CODE = M.ITEM_CODE
		...
		AS
			(
				...
				SELECT distinct
						...
						'0' AS vir_item_flag,
						AM.in_cust																																								AS GOLE_IN_CUST_CODE,			/*최종납품 업체*/
						IIF(AM.in_cust>'','',IIF(AM.WORK_CODE='P2', 'IS0001', (SELECT TOP 1 GAGONG_PROC_CODE FROM nx.prodinfo_proc WITH (NOLOCK) WHERE ITEM_CODE = T.ITEM_CODE AND PROC_SEQ = 1)))	AS GOLE_GAGONG_PROC_CODE,		/*최종납품 생산파트*/
						AM.WORK_CODE,
						AM.ITEM_CLASS
				  FROM #TEMP_PART_DTL	T
				  join nx.item			am WITH (NOLOCK) on T.item_code = am.item_code
				 where t.proc_seq <= 1
				...
				SELECT cb.item_code,
						...
						b.vir_item_flag,
						IIF(CB.VIR_ITEM_FLAG='1', CB.GOLE_IN_CUST_CODE, AM.in_cust)												AS GOLE_IN_CUST_CODE,			/*최종납품 업체*/
						IIF(CB.VIR_ITEM_FLAG='1', CB.GOLE_GAGONG_PROC_CODE, IIF(AM.in_cust>'','',(SELECT GAGONG_PROC_CODE FROM nx.prodinfo_proc WITH (NOLOCK) WHERE ITEM_CODE = B.ITEM_CODE AND PROC_SEQ = 1))) AS GOLE_GAGONG_PROC_CODE,		/*최종납품 생산파트*/
						M.WORK_CODE,
						M.ITEM_CLASS
				  FROM CTE_BOM 			cb
				  join pr_m_item_bom 	b WITH (NOLOCK) on cb.mat_code	= b.item_code
				  join nx.item 			am WITH (NOLOCK) on b.item_code = am.item_code
				  join nx.item 			m WITH (NOLOCK) on b.mat_code 	= m.item_code
				 where isnull(b.except_flag,'0')='0'							/*전개제외*/
				   AND (B.VIR_ITEM_FLAG = '1' OR M.in_cust > '' OR M.WORK_CODE = 'P2' OR NOT EXISTS (SELECT * FROM nx.prodinfo_proc WITH (NOLOCK) WHERE ITEM_CODE = B.MAT_CODE))		/*하위품이 사내생산이 아닌 것*/
			)	
		/*대상 상세자재 추출*/
		...
		AS
			(
				...
				SELECT distinct
						...
						b.vir_item_flag,
						AM.in_cust																																		AS GOLE_IN_CUST_CODE,			/*최종납품 업체*/
						IIF(AM.in_cust>'','',(SELECT TOP 1 GAGONG_PROC_CODE FROM nx.prodinfo_proc WITH (NOLOCK) WHERE ITEM_CODE = B.ITEM_CODE AND PROC_SEQ = 1))	AS GOLE_GAGONG_PROC_CODE,		/*최종납품 생산파트*/
						M.WORK_CODE,
						M.ITEM_CLASS
				  FROM #TEMP_PART_DTL	T
	...
	*/
	...
	BEGIN
		...
		join nx.item b WITH (NOLOCK) on b.item_code			= a.assy_item_code
	...
	BEGIN
		select t.*,
				...
				ISNULL((select cust_name FROM nx.cust WITH (NOLOCK) WHERE CUST_CODE = T.GOLE_IN_CUST_CODE),'') AS GOLE_IN_CUST_DESC,
