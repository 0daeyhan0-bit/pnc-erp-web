...
class NoCommitConn:
    """공유 트랜잭션 커넥션 — 라우터가 commit/close 해도 무력화."""
...
for name, mod in list(sys.modules.items()):
    ...
    for attr in ('_nx', '_nx_tx'):
        if hasattr(mod, attr):
            setattr(mod, attr, lambda: SHARED); _patched += 1
...
PROBES = [
    ...
    ("용접봉차감",  "SELECT ISNULL(SUM(CAST(MAINT_QTY AS float)),0) FROM nx.stock_ledger WHERE MAINT_YMD=? AND STOCK_POINT='MAT' AND MAINT_TAG='W'"),
    ("용접링차감",  "SELECT ISNULL(SUM(CAST(MAINT_QTY AS float)),0) FROM nx.stock_ledger WHERE MAINT_YMD=? AND STOCK_POINT='MAT' AND MAINT_TAG='R'"),
...
def _probe():
    ...
    for nm, q in PROBES:
        try:
            cur.execute(q, _SCOPE["ymd"]); out[nm] = float(cur.fetchone()[0] or 0)
        except Exception:
            out[nm] = None
    for nm, q in PROBES_MAT:
        try:
            cur.execute(q, _SCOPE["mat"]); out[nm] = float(cur.fetchone()[0] or 0)
        except Exception:
            out[nm] = None
    return out
