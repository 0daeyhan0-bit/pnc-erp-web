...
def export_single_pbl_100pct(h_session, pbl_path, base_out_dir):
    ...
    def dir_callback(p_entry_info, p_user_data):
        try:
            ...
            # Find object name at offset 784 or scan
            entry_name = ""
            for off in range(0, 1000, 2):
                wstr = raw_bytes[off:off+300].decode('utf-16-le', errors='ignore').split('\x00')[0]
                if wstr and not wstr.startswith((' ', '\t', '\r', '\n')) and len(wstr) > 1 and re.match(r'^[a-zA-Z0-9_가-힣]+$', wstr):
                    if wstr.startswith(('w_', 'dw_', 'u_', 'm_', 'f_', 'd_', 'p_', 'cs_', 'pr_', 'sa_', 'pu_', 'hr_', 'ma_', 'qa_', 'cm_', 'app_', 'partnererp', 'ds_')):
                        entry_name = wstr
                        break
            if not entry_name:
                # fallback offset 784
...
def main():
    ...
    # Pass ALL PBLs to session library list
    ...
    for i, pbl_path in enumerate(sorted(pbl_files)):
        pbl_name = os.path.basename(pbl_path)
        try:
            succ, total = export_single_pbl_100pct(h_session, pbl_path, base_out_dir)
            total_objects += succ
    ...
    print("\n==========================================")
    print("100% ALL PBL SOURCE EXPORT COMPLETED!")
