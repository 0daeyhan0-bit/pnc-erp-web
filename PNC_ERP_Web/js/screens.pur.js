  ...
  const render=()=>{
    ...
    host.innerHTML=`
     ...
     <div class="grid-wrap" style="${st.workMode?'display:none;':''}max-height:calc(100vh - 250px);overflow:auto;background:#fff;border:1px solid var(--line-2,#c9d3e0);border-radius:8px">
      ...
      <tbody>${st.loading?spinRow(canEd?18:16):(st.rows.length?st.rows.map((r,i)=>`<tr class="cq-row" data-idx="${i}" style="cursor:pointer">
         ...
         ${be.loading?'<div style="padding:40px;text-align:center;color:#8aa0bd">불러오는 중…</div>':(!d?(be.isNew?`<div style="padding:34px;text-align:center"><div style="margin-bottom:12px;color:#33507d;font-size:13px">품번을 입력하면 <b>현 BOM 구성</b>이 자동으로 펼쳐집니다.</div><input class="inp" id="be-newitem" placeholder="Assy 품번" style="width:230px;font-family:monospace" value="${esc(be.assy||'')}"> <button class="btn" id="be-load" style="background:#1c7c3a;color:#fff">🔍 BOM 불러오기</button></div>`:'<div style="padding:40px;text-align:center;color:#c0392b">BOM 조회 실패</div>'):`
           ...
           <tbody><tr style="background:#eaf1fc;font-weight:700">
             ...
             return `<tr style="${need?'background:#fdf0f0':''}">
               ...
               <td>${beBadge(r.role)}</td><td class="num">×${nf4(r.cum_qty)}</td>
               ...
               <td class="num be-uw">${uw?nf4(uw):(r.role==='용접봉'?'<span style="color:#b8791f;font-size:9px">공정</span>':'-')}</td>
               ...
               <td class="num" style="${r.in_quote===false?'color:#c9d1dc;font-weight:400':'color:#c0392b;font-weight:700'}" title="${r.in_quote===false?'합계 비산입 (상위 견적 덩어리에 포함됨)':'현재 재료비 (사급=판매단가·동관=소요중량×사급가·용접봉=소요×단가)'}">${r.mat_now!=null?(r.in_quote===false?'('+nf(r.mat_now)+')':nf(r.mat_now)):'-'}</td>
               ...
               <td class="num" style="color:#c0392b;font-weight:700">${r.role==='용접봉'?nf(r.weld_cost)+'<span style="font-size:9px;color:#8a6d3b"> 재료</span>':(r.part_total!=null?nf(r.part_total):'<span style="color:#c9d1dc">-</span>')}</td></tr>`;}).join('')}
           ...
           <div style="margin-top:8px;background:#eef4ff;border:1px solid #cdddf5;border-radius:8px;padding:9px 14px;font-size:12px;display:flex;gap:12px;justify-content:flex-end;align-items:center;flex-wrap:wrap">
             ...
             <span style="color:#8aa0bd">+ 가공비 <b style="color:#1c7c3a">${nf(proc)}</b> =</span>
             <span>판가 <b id="be-tsale" style="font-size:14px">${nf(sale)}</b></span></div>
           ${(d.total_proc_cost>0)?`<div style="margin-top:6px;font-size:12px;color:#8a6d3b;background:#fff8ec;border:1px solid #f0e0c0;border-radius:6px;padding:7px 12px">💡 현재 판가(<b>${nf(sale)}</b>)를 <b>공정 계산</b>으로 맞추려면 임율 = <b id="be-reqrate">${nf(Math.round((sale-mat)*d.labor_rate/d.total_proc_cost))}</b>원 필요 <span style="color:#b0a080">(현재 임율 ${nf(d.labor_rate)}원 · 공정계산 가공비 ${nf(d.total_proc_cost)})</span></div>`:''}`)}
         </div>
    ...
    host.querySelectorAll('.cq-row').forEach(tr=>tr.onclick=()=>loadParts(+tr.dataset.idx));
    if(st.detail){const dc=g('#cq-dc'),dx=g('#cq-dx');const close=()=>{st.detail=null;render();};if(dc)dc.onclick=close;if(dx)dx.onclick=close;}
    if(modal){
    ...
    if(be){
      ...
      host.querySelectorAll('.be-sp').forEach(inp=>inp.oninput=()=>{
        const code=inp.dataset.code,fld=inp.dataset.f;be.edits[code]=be.edits[code]||{};be.edits[code][fld]=inp.value;
        const tr=inp.closest('tr');const r=be.data.rows.find(x=>x.code===code);const uw=beUw(r);
        const uwc=tr.querySelector('.be-uw'),swc=tr.querySelector('.be-sw');
        if(uwc)uwc.textContent=uw?nf4(uw):'-';if(swc)swc.textContent=uw?nf4(uw*r.cum_qty):'-';
        tr.style.background=uw?'':'#fdf0f0';upd();});
      host.querySelectorAll('.be-pc').forEach(inp=>inp.oninput=()=>{
        const code=inp.dataset.code,op=inp.dataset.op;be.procEdits[code]=be.procEdits[code]||{};be.procEdits[code][op]=inp.value;
        const r=be.data.rows.find(x=>x.code===code);const rg=beRowGagong(r);
        const span=inp.closest('tr').querySelector('.be-rg');if(span)span.textContent=nf(rg);});
      const asmUpd=()=>{if(!be.asm)return;const A=be.asm;const atot=Math.round((+A.gagong||0)+(+A.mgmt||0)+(+A.transport||0)+(+A.profit||0));
        const t=g('#be-asmtot');if(t)t.textContent=nf(atot);const grand=(be.data.part_sum||0)+atot;
        const gl=g('#be-grandtot-l'),gt=g('#be-grandtot');if(gl)gl.textContent=nf(grand);if(gt)gt.textContent=nf(grand);};
