...
SCREEN.sourceprofile=(c)=>{
  ...
  // ===== 업체·단가 모달 — ★가격=SUB/품목 공통(업체 무관), 업체=배분%만 =====
  // 1) ASSY 매입단가=외주 SUB당 1개(nx.sourcing_sub_price PK route_id,sub_item) 2) 사급 부품가=매입 부품당 1개(nx.sourcing_sagub_price PK route_id,item_code)
  // 3) 단품 매입=매입 마스터 자동(입력X·읽기전용). ★후보/계획 단가(정산 아님): nx만 저장. 정산 마스터 불변(마감때만).
  let pm=null, pmAcT=null;   // pm={route,hdr,rows[](업체=배분%만),subs[],subChildren{},assy{}(sub_item→가),sagub{}(item_code→가),direct[],msg,loading}
  ...
  const pmOpen=async(r)=>{pm={route:r,hdr:null,rows:[],subs:[],subChildren:{},assy:{},sagub:{},direct:[],msg:'',loading:true};draw();
  ...
  const pmSave=async()=>{
    ...
    // ② ASSY 매입단가(외주 SUB당 1개·공통) ③ 사급 부품가(매입 부품당 1개·공통)
    const arows=pm.subs.map(s=>({sub_item:s.sub_item,assy_price:(pm.assy[s.sub_item]!==''&&pm.assy[s.sub_item]!=null)?parseFloat(pm.assy[s.sub_item]):null}));
    const grows=[];Object.keys(pm.subChildren).forEach(si=>pm.subChildren[si].filter(ch=>ch.is_purchase).forEach(ch=>{
      const v=pm.sagub[ch.item_code];grows.push({item_code:ch.item_code,sagub_price:(v!==''&&v!=null)?parseFloat(v):null});}));
  ...
  const vendorModal=()=>{if(!pm)return '';const S=pmStat(),ok=S.single||Math.abs(S.sum-100)<0.01;
    ...
    // ② 외주 SUB 블록: ASSY 매입단가 1개(공통) + 사급 부품가 목록(매입 부품별 1개·공통)
    const subBlocks=pm.subs.map(s=>{
      ...
      return `<div style="margin-top:10px;border:1px solid #e6d29a;border-radius:8px;overflow:hidden">
        <div style="padding:6px 10px;background:#fdf7e6;font-size:12px;font-weight:700;color:#8a6d1c;display:flex;align-items:center;gap:8px">
          ...
          <span style="font-weight:600;color:#8a6d1c">ASSY 매입단가</span>
        ...
        <tbody>${chHtml}</tbody></table></div>`;}).join('');
    ...
    return `<div class="wr-modal" style="position:fixed;inset:0;z-index:120;background:rgba(20,30,50,.42);display:flex;align-items:flex-start;justify-content:center;overflow:auto;padding:24px 10px">
      <div style="background:#fff;border-radius:10px;min-width:760px;max-width:96vw;box-shadow:0 8px 40px rgba(0,0,0,.25)">
       <div style="padding:12px 16px;border-bottom:1px solid #e2e8f2;display:flex;align-items:center;gap:10px">
         <span style="font-weight:700;font-size:15px;color:#1c3a6e">🏭 업체·단가 지정 <span style="font-size:11px;font-weight:400;color:#8aa0bd">(가격=SUB/품목 공통 · 업체=배분%)</span></span>
       ...
       <div style="padding:8px 16px 4px;font-size:12px;color:#8a6d1c;background:#fdf7e6;border-bottom:1px solid #f0e6c8">
         ⚠️ <b>후보/계획 단가(정산 아님)</b> · <b>가격은 업체 무관(SUB/품목당 1개 공통)</b>: ① <b>ASSY 매입단가</b>=외주 SUB당 1개 ② <b>사급 부품가</b>=매입 부품당 1개(제작=원가 자동·입력X) ③ <b>단품 매입품</b>=매입 마스터 자동(읽기전용). 업체는 <b>배분%(공급능력)</b>만. 정산 매입/판매 단가(마감때만 수정)는 변경되지 않습니다.</div>
       <div style="padding:0 16px 12px;overflow:auto;max-height:66vh">
         <div style="font-size:12px;font-weight:700;color:#334;margin:12px 0 0">② 외주 SUB — ASSY 매입단가 · 사급 부품가 <span style="font-weight:400;font-size:11px;color:#8aa0bd">(가격은 업체 공통)</span></div>
         ${subBlocks}${noSub}
         ${directBlock}
