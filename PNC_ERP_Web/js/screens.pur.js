...
SCREEN.manorder=(c)=>{
  ...
  const draw=()=>{
    c.innerHTML=`
     ...
     :`
       <div class="toolbar" style="margin-top:2px;flex:0 0 auto"><span class="rowcount" id="mo-sum"></span><label style="margin-left:12px;color:#c0392b;font-size:12px">발주리드타임</label><input class="inp" id="mo-lead" type="number" min="0" max="365" value="${lead}" style="width:46px;text-align:right;border-color:#c0392b;color:#c0392b" title="이 일수 이내 계획분은 발주로 못 바꿈(추가발주에서 차감)">일<button class="btn ghost" id="mo-exp" style="margin-left:10px;padding:2px 8px;font-size:12px">${expand?'▸ 주별 접기':'▾ 일별 펼치기'}</button><span style="margin-left:10px;color:#8aa0bd;font-size:12px">☑ 발주할 품목 체크 · 추가발주 직접조정</span></div>
       <div style="flex:1;min-height:0;display:flex;gap:10px;align-items:stretch">
         <div style="flex:0 0 44%;min-width:0;display:flex;flex-direction:column;min-height:0">
           ...
           <div class="grid-wrap" id="mo-lwrap" style="flex:1;min-height:0;overflow:auto;background:#fff;border:1px solid var(--line-2,#c9d3e0);border-radius:8px">
           <table class="tbl fit" id="mo-ltbl" style="font-size:12px;width:100%"><thead><tr><th class="center" style="width:28px">No</th><th class="center" style="width:24px"><input type="checkbox" id="mo-all" checked title="전체선택"></th><th style="width:92px">품목</th><th style="min-width:100px">품명</th><th class="num" style="width:46px">현재고</th><th class="num" style="width:46px">기발주</th>${expand?planDates.map(d=>`<th class="num" style="white-space:nowrap;font-size:11px;min-width:40px">${esc((''+d).slice(2,4))}/${esc((''+d).slice(4,6))}</th>`).join(''):`<th class="num" style="min-width:42px">1주</th><th class="num" style="min-width:42px">2주</th><th class="num" style="min-width:42px">3주</th><th class="num" style="min-width:42px">4주</th>`}<th class="num" style="white-space:nowrap;background:#f3f6fa;width:54px" title="LG 물동 5~8주 예상소요 — 참고용(부정확). 4주는 생산계획, 5~8주는 LG물동. 발주계산 미반영·담당 판단.">LG물동<br><span style="color:#8a94a6;font-size:10px">(5~8주)</span></th><th class="num" style="white-space:nowrap;width:64px">여유분<br><input class="inp" id="mo-buf" type="number" min="0" max="999" value="${buf}" style="width:42px;min-width:42px;text-align:right;padding:2px 2px">%</th><th class="num" style="width:80px">추가발주</th></tr></thead>
           <tbody>${loading?spinRow(10):(items.length?items.map((it,i)=>{const a=ord(it);const wk=it.week_qty||[0,0,0,0];return `<tr><td class="center mut">${i+1}</td><td class="center"><input type="checkbox" class="mo-ck" data-ic="${esc(it.ic)}" ${a>0?'checked':''}></td><td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(it.ic)}${it.alloc_note?' · '+esc(it.alloc_note):''}"><b>${esc(it.ic)}</b>${it.alloc_note?'<span style="color:#7a4ca0;font-size:9px" title="조달 프로파일 배분/발주업체지정 — 이 매입처 몫만 계상"> ▸배분</span>':''}</td><td title="${esc(it.nm)}" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:150px">${esc(it.nm)}</td>
             <td class="num">${nf(it.stock_qty)}</td><td class="num">${nf(it.po_qty)}</td>${expand?planDates.map(d=>{const q=(it.days&&it.days[d])||0;return `<td class="num">${q?nf(q):''}</td>`;}).join(''):wk.map(q=>`<td class="num">${q?nf(q):'<span style="color:#dbe2ea">·</span>'}</td>`).join('')}<td class="num" style="color:#8a94a6;font-style:italic;background:#fafcff" title="LG물동 5~8주 참고소요 — 발주계산 미반영(담당 판단)">${it.muldong_soyo>0?nf(it.muldong_soyo):'<span style="color:#d0d8e2">-</span>'}</td><td class="num" style="color:#8aa0bd" title="여유분 ${buf}% (반영 ${lead}일치 계획 ${nf(adjPlan(it))} 기준)">${nf(bufQty(it))}</td>
             <td class="num" title="추가발주 = max(0, 반영 ${lead}일치 계획 ${nf(adjPlan(it))} ×(1+${buf}%) − 현재고 ${nf(it.stock_qty)} − 기발주 ${nf(it.po_qty)})"><input class="mo-add" data-ic="${esc(it.ic)}" type="number" min="0" value="${a}" style="width:74px;text-align:right;font-weight:700;color:#1c7c3a"></td></tr>`;}).join(''):`<tr><td colspan="${9+(expand?planDates.length:4)}" class="empty">품목 없음</td></tr>`)}</tbody></table></div>
         </div>
         <div style="flex:1;min-width:0;display:flex;flex-direction:column;min-height:0">
           <div style="font-weight:700;margin:2px 0 4px;color:#1c47a0;flex:0 0 auto">협력사 일자별 계획 <span style="font-weight:400;color:#8aa0bd;font-size:12px">${esc(vendor.nm)} · ${esc(vendor.ym||'-')} (좌측과 순번 1:1)</span></div>
    ...
    const ex=c.querySelector('#mo-exp');if(ex)ex.onclick=()=>{expand=!expand;draw();};   // 주별↔일별 펼치기
