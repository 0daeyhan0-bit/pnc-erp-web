...
SCREEN.manorder=(c)=>{
  ...
  const draw=()=>{
    c.innerHTML=`
     ...
     :`
       <div class="toolbar" style="margin-top:2px;flex:0 0 auto"><span class="rowcount" id="mo-sum"></span><span style="margin-left:12px;color:#8aa0bd;font-size:12px">☑ 발주할 품목 체크 · 추가발주 수량 직접 조정 가능</span></div>
       <div style="flex:1;min-height:0;display:flex;gap:10px;align-items:stretch">
         <div style="flex:0 0 58%;min-width:0;display:flex;flex-direction:column;min-height:0">
           ...
           <table class="tbl fit" id="mo-ltbl" style="font-size:12px;width:100%;table-layout:fixed"><thead><tr><th class="center" style="width:30px">No</th><th class="center" style="width:26px"><input type="checkbox" id="mo-all" checked title="전체선택"></th><th style="width:118px">품목</th><th style="width:80px">품명</th><th class="num" style="white-space:nowrap">계획수량<br><span style="color:#c0392b;font-size:10px" title="발주 리드타임 — 이 일수 이내 계획분은 발주로 못 바꾸므로 차감(기본 2주)"><input class="inp" id="mo-lead" type="number" min="0" max="365" value="${lead}" style="width:52px;min-width:52px;text-align:right;padding:1px 4px;border-color:#c0392b;color:#c0392b">일</span></th><th class="num" style="white-space:nowrap;background:#f3f6fa" title="LG 물동 5~8주 예상소요 — 참고용(부정확). 4주는 생산계획, 5~8주는 LG물동. 발주계산 미반영·담당 판단.">LG물동<br><span style="color:#8a94a6;font-size:10px">${vendor&&vendor.muldong_ym?esc(vendor.muldong_ym):'5~8주'}·참고</span></th><th class="num">기발주</th><th class="num">현재고</th><th class="num" style="white-space:nowrap;width:52px">여유분<br><input class="inp" id="mo-buf" type="number" min="0" max="999" value="${buf}" style="width:38px;min-width:38px;text-align:right;padding:2px 3px">%</th><th class="num" style="width:84px">추가발주</th></tr></thead>
           <tbody>${loading?spinRow(10):(items.length?items.map((it,i)=>{const a=ord(it);return `<tr><td class="center mut">${i+1}</td><td class="center"><input type="checkbox" class="mo-ck" data-ic="${esc(it.ic)}" ${a>0?'checked':''}></td><td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(it.ic)}"><b>${esc(it.ic)}</b></td><td class="bcap" title="${esc(it.nm)}" style="max-width:96px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(it.nm)}</td>
