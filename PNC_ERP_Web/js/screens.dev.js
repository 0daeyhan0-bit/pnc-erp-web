...
SCREEN.price=(c)=>{
  ...
  const paint=()=>{
    ...
    if(mode==='item') priceItemView(host); else if(mode==='hist') priceHistView(host); else if(mode==='inv') priceInvView(host); else priceSagubView(host);
  };
  paint();
};
...
SCREEN.costanalysis=(c)=>{
  ...
  const NUM=[[1,'입고수량','',{}],
    ...
    [8,'원자재비','실원가',{}],[9,'부자재비','실원가',{}],[10,'LG사급비','실원가',{}],[11,'재료비합계','실원가',{}],[12,'실원가','실원가',{b:1}],[13,'가공비','실원가',{}],[14,'일반관리','실원가',{}],[15,'운반비','실원가',{}],[16,'이윤','실원가',{}],[17,'사급차액','실원가',{sk:1}],[18,'재료비율','실원가',{pct:1}],
  ...
  const draw=()=>{
    c.innerHTML=`
     ...
     <style>
       ...
       .ca-tbl thead th{height:23px;box-sizing:border-box;white-space:nowrap;background:#f4f7fc}
       .ca-tbl thead tr:first-child th{position:sticky;top:0;z-index:4}
       .ca-tbl thead tr:nth-child(2) th{position:sticky;top:23px;z-index:4}
