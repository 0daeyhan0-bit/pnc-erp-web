...
SCREEN.unifybom=(c,ro)=>{
  ...
  const drawNae=()=>{
    ...
    // 팝업(모달)
    {const x=g('#pm-close');if(x)x.onclick=()=>{naeModal=false;drawNae();};}
    {const bg=g('#pm-backdrop');if(bg)bg.onclick=e=>{if(e.target===bg){naeModal=false;drawNae();}};}
  ...
  const naeLevelTree=(a,rows)=>{
    ...
    const body=rowsT.map(r=>{const sp=r.diam?('Ø'+r.diam+(r.thick?'×'+r.thick:'')):(r.spec||'');const sel=naeSel===r.code;
      ...
      return `<tr class="nae-trow" data-node="${esc(r.code)}" data-level="${r.level}" style="cursor:pointer;background:${sel?'#e8f0ff':bg}">
        ...
        <td class="center"><button class="btn xs nae-edit-btn" data-node="${esc(r.code)}" data-level="${r.level}" style="padding:0 7px;background:#8e44ad;color:#fff">등록/수정</button></td></tr>`;}).join('')||'<tr><td colspan=6 class="empty">구성 없음 — 조회하세요</td></tr>';
    return `<div style="display:flex;flex-direction:column;min-height:0;height:100%">
      <div class="summary-bar" style="flex:0 0 auto"><div class="s-item"><b>BOM 레벨트리</b> · 레벨0=제품 / 레벨1+=부품 · 행클릭=우측조회 · <b>[등록/수정]</b>=공정입력 팝업</div></div>
