...
SCREEN.partmaster=(c)=>{
  const API=API_BASE;
  const st={rows:[],cnt:0,q:'',loading:false,msg:'',edit:null,sel:'',workers:[],wload:false};
  ...
  const draw=()=>{
    ...
    c.innerHTML=`
     ...
     <div style="display:flex;gap:10px;align-items:flex-start">
      ...
      <div style="flex:1 1 0;min-width:360px;max-width:520px">
       <div style="font-weight:700;font-size:13px;color:#33507d;padding:4px 2px">👷 파트별 작업자 ${st.sel?`— <b style="color:#1c47a0">${esc(st.sel)}</b> <span style="color:#8aa0bd;font-weight:400">${st.wload?'…':st.workers.length+'명'}</span>`:'<span style="color:#8aa0bd;font-weight:400">— 좌측 파트 클릭</span>'}</div>
