...
def _incost(cur, assys):
    ...
    #   (갱신: /api/coopquote2/refresh-incost 또는 populate_incost.py)
...
@router.post("/api/coopquote2/refresh-incost")
...
@router.get("/api/coopquote2/vendors")
def coopquote_vendors():
    nx = _nx(); cur = nx.cursor()
    try:
        cur.execute("SELECT vendor, COUNT(*) n FROM nx.coop_quote_v2 GROUP BY vendor ORDER BY vendor")
...
def coopquote_list(vendor: str = Query(""), q: str = Query(""), active_only: int = Query(0), newonly: int = Query(0),
    ...
    try:
        ...
        cur.execute(f"""SELECT quote_id,vendor,assy_code,item_name,spec,total_weight,sagub_price,
          ...
          FROM nx.coop_quote_v2 WHERE {' AND '.join(where)}
        ...
        for i in range(0, len(assy_up), 400):
            ...
            cur.execute(f"""SELECT DISTINCT UPPER(LTRIM(RTRIM(assy_code))) FROM nx.coop_quote_part_v2
                WHERE UPPER(LTRIM(RTRIM(assy_code))) IN ('{inlist}') AND ISNULL(is_active,1)=1 AND (
                    (ptype_v2 IN (N'사급',N'사급받음',N'용접봉',N'수불') AND (mat_after IS NULL OR mat_after=0))
                    OR (ptype_v2=N'용접봉' AND part_code NOT LIKE 'RAC%' AND part_code NOT LIKE '3H%')   -- 용접봉 오분류(잡자재910)
                )""")
...
@router.get("/api/coopquote2/parts")
def coopquote_parts(assy: str = Query(...), vendor: str = Query("")):
    ...
    try:
        ...
        cur.execute(f"""SELECT seq,part_code,part_name,ptype,spec,mat_cost,proc_cost,part_total
            FROM nx.coop_quote_part_v2 WHERE {' AND '.join(w)} ORDER BY seq""", *a)
...
@router.post("/api/coopquote2/set-role")
def coopquote_set_role(payload: dict = Body(...)):
    ...
    try:
        ...
        cur.execute("SELECT ISNULL(sale_price,0) FROM nx.coop_quote_v2 WHERE assy_code=?", assy)
        sr = cur.fetchone(); sale = float(sr[0] or 0) if sr else 0
        proc = max(0, sale - mat_cost)
        cur.execute("""UPDATE nx.coop_quote_v2 SET mat_cost=?, mat_raw=?, mat_weld=?, mat_part=?, total_weight=?, proc_cost=?, upd_dt=GETDATE()
...
@router.post("/api/coopquote2/save")
def coopquote_save(payload: dict = Body(...)):
    ...
    try:
        if qid:
            cur.execute("""UPDATE nx.coop_quote_v2 SET vendor=?,assy_code=?,item_name=?,spec=?,
        ...
        cur.execute("""INSERT INTO nx.coop_quote_v2(vendor,assy_code,item_name,spec,total_weight,sagub_price,
...
@router.post("/api/coopquote2/recalc")
    ...
    try:
        ...
        for grade, price in (("일반CU", pn), ("고강도CU", ph)):
            if not price or price <= 0:
                continue
            cur.execute(f"""UPDATE nx.coop_quote_v2
...
@router.get("/api/coopquote2/bom-form")
def coopquote_bom_form(item: str = Query(..., description="품번(Assy)"), vendor: str = Query(""),
    ...
    try:
        ...
        cur.execute("""WITH tree AS (
            ...
            FROM PARTNER_ERP_TEST3.nx.v_cs_bom WHERE ITEM_CODE=? AND FROM_APPLY_YMD<='991231' AND TO_APPLY_YMD>='260101' AND ISNULL(CS_CALC_EXCEPT_FLAG,'')<>'1'
            UNION ALL
            SELECT b.ITEM_CODE, b.MAT_CODE, CAST(b.USE_QTY AS decimal(18,6)), ISNULL(b.SAGUB_FLAG,'0'), ISNULL(b.BOM_SEQ,0), t.lvl+1
            FROM tree t JOIN PARTNER_ERP_TEST3.nx.v_cs_bom b ON b.ITEM_CODE=t.c AND b.FROM_APPLY_YMD<='991231' AND b.TO_APPLY_YMD>='260101' AND ISNULL(b.CS_CALC_EXCEPT_FLAG,'')<>'1'
            WHERE t.lvl < 8)
    ...
    try:
        ...
        try:
            ncur.execute("SELECT UPPER(LTRIM(RTRIM(part_code))), part_total, mat_cost, proc_cost, ISNULL(ptype,'') FROM nx.coop_quote_part_v2 WHERE assy_code=? ORDER BY seq", item)
            for r in ncur.fetchall():
            ...
            ncur.execute("SELECT ISNULL(sale_price,0), ISNULL(mat_cost,0), ISNULL(grade,N'일반CU'), ISNULL(sagub_price,0) FROM nx.coop_quote_v2 WHERE assy_code=?", item)
            rr = ncur.fetchone()
            if rr:
    ...
    _ROLE = {'사급부품': '사급', '자작': '사급', '무상': '사급', '기존가': '사급', '매입부품': '매입부품',
             ...
             # ptype_v2(정산분류) 값도 매핑 — 수불=제작동관(중량×사급가), 사급/사급받음=사급(mat_now 직접)
             '수불': '제작동관', '사급': '사급', '사급받음': '사급'}
    for r in rows:
...
@router.post("/api/coopquote2/bom-save")
def coopquote_bom_save(payload: dict = Body(...)):
    ...
    nx = _nx(); cur = nx.cursor()
    try:
        cur.execute("SELECT quote_id FROM nx.coop_quote_v2 WHERE assy_code=? AND vendor=?", item, vendor)
        row = cur.fetchone()
        nm = str(payload.get("item_name") or "")[:120]; spec = str(payload.get("spec") or "")[:80]
        if row:
            cur.execute("""UPDATE nx.coop_quote_v2 SET item_name=?,spec=?,total_weight=?,sagub_price=?,mat_cost=?,
...
@router.get("/api/coopquote2/worklist")
