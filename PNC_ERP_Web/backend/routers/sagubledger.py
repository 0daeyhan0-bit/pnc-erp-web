...
def sagubledger_list(request: Request, cust: str = Query(""), mat: str = Query(""), fr: str = Query(""),
    ...
    try:
        ...
        # 협력사 목록 = 원장에 실제 거래(입고/출고)가 있는 협력사만(사용자 확정 2026-08-29).
        #   거래 없는 협력사(예 동주금속=사급 0건)는 노출 안 함. 협력사 로그인 시 자기 것만은 소속 강제로 처리.
        cur.execute("""SELECT DISTINCT LTRIM(RTRIM(l.cust_code)), LTRIM(RTRIM(ISNULL(c.CUST_DESC,''))) nm
            FROM nx.sagub_parts_ledger l LEFT JOIN nx.CM_M_CUST c ON c.CUST_CODE=l.cust_code ORDER BY nm""")
