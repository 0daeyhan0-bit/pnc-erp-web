...
# INSP_FLAG (varchar(1)) — 실측 분포: F 40,372 · NULL 18,663 · N 9,863 · S 1,542 · '' 575
#   ★라벨 정정(2026-08-31): 레거시 원화면 드롭다운이 정답이다.
#       F:유검사 · S:체크검사 · N:무검사
#     종전 "F 전수검사 / S 샘플검사"는 추정이었다(주석에도 '라벨 잠정'이라 적혀 있었다).
#     코드는 그대로 3종이고 새 코드를 만들 필요가 없다.
#   ★업무규칙(대표 확정): 유검사(F)·체크검사(S) 는 IQC 검사완료가 있어야 자재입고된다.
#     코드베이스의 게이트가 이미 IN ('S','F') 라 규칙과 일치한다(추가 작업 없음).
_INSP = {"F": "유검사", "S": "체크검사", "N": "무검사"}
...
def itemmaster_get(item: str = Query(...)):
    ...
    try:
        ...
        cur.execute("SELECT axis_procure, axis_produce, axis_sales, axis_sales_code, unclassified, procure_method FROM nx.v_item_axis3 WHERE item_code=?", code)
        ra = cur.fetchone()
        axis3 = {"procure": ra[0] or "", "produce": ra[1] or "", "sales": ra[2] or "",
                 "sales_code": ra[3] or "", "unclassified": bool(ra[4]), "method": ra[5] or ""} if ra else {}
