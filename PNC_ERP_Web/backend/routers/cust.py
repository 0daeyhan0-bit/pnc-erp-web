...
def cust_opts():
    ...
    try:
        cur.execute("SELECT DETAIL_CODE, DETAIL_DESC FROM PARTNER_ERP_TEST3.nx.v_code_detail WHERE KIND_CODE='PR011' ORDER BY DETAIL_CODE")
        cust_type = [{"code": str(r[0]).strip(), "nm": str(r[1]).strip()} for r in cur.fetchall()]
        cur.execute("SELECT DETAIL_CODE, DETAIL_DESC FROM PARTNER_ERP_TEST3.nx.v_code_detail WHERE KIND_CODE='CM701' ORDER BY DETAIL_CODE")
...
def cust_list(q: str = Query(""), use: str = Query(""), ctype: str = Query("")):
    ...
    try:
        cur2 = _conn().cursor()
        cur2.execute("SELECT DETAIL_CODE, DETAIL_DESC FROM PARTNER_ERP_TEST3.nx.v_code_detail WHERE KIND_CODE='PR011'")
