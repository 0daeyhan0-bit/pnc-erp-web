...
def _nx_derive(point, from6, to6, limit=8000):
    ...
    sql = f"""SELECT TOP {int(limit)}
      ...
      LEFT JOIN PARTNER_ERP.dbo.PR_M_ITEM i ON i.ITEM_CODE=COALESCE(NULLIF(L.MAT_CODE,''),L.ITEM_CODE)
      WHERE L.STOCK_POINT=?
    ...
    gmap, cmap = {}, {}
    if gpcs:
        _c, gr = _rows(f"SELECT gagong_proc_code cd, gagong_proc_desc nm FROM PR_M_PROC_GAGONG WHERE gagong_proc_code IN ({','.join(_q(x) for x in gpcs)})")
        gmap = {str(x["cd"]).strip(): (x["nm"] or "") for x in gr}
    if custs:
        _c, cr = _rows(f"SELECT cust_code cd, cust_desc nm FROM CM_M_CUST WHERE cust_code IN ({','.join(_q(x) for x in custs)})")
...
def matledger(period: str = Query("day"), ymd: str = Query(""), source: str = Query("live")):
    ...
    if period == "month":
        ...
        if len(key) >= 6:  # YYYYMM → YYMM
            key = key[2:6]
        if not key:
            key = _scalar("SELECT MAX(STOCK_YYMM) FROM PU_T_MONTH_STOCK_WH WHERE cust_code='Z99990'")
        if source == "nx":
        ...
        if len(key) == 8:  # YYYYMMDD → YYMMDD
            key = key[2:]
        if not key:
            key = _scalar("SELECT MAX(STOCK_YMD) FROM PU_T_MONTH_STOCK_WH_DAILY WHERE cust_code='Z99990'")
        if source == "nx":
...
def matledger_dates():
    ...
    dmax = _scalar("SELECT MAX(STOCK_YMD) FROM PU_T_MONTH_STOCK_WH_DAILY WHERE cust_code='Z99990'")
    dmin = _scalar("SELECT MIN(STOCK_YMD) FROM PU_T_MONTH_STOCK_WH_DAILY WHERE cust_code='Z99990'")
    mmax = _scalar("SELECT MAX(STOCK_YYMM) FROM PU_T_MONTH_STOCK_WH WHERE cust_code='Z99990'")
    mmin = _scalar("SELECT MIN(STOCK_YYMM) FROM PU_T_MONTH_STOCK_WH WHERE cust_code='Z99990'")
...
def _dispatch(dc, ref_ym):
    magam = f"""WITH MAGAM (CUST_CODE, JUN_YYMM, JUN_MAGAM_DAY, MAGAM_DAY) AS (
      SELECT CUST_CODE
        ...
        ,ISNULL((SELECT TOP 1 MAGAM_DAY FROM CM_M_CUST_MAGAM WHERE CUST_CODE=A.CUST_CODE AND APPLY_YYMM<=format(dateadd(MONTH,-1,convert(date,'{ref_ym}'+'01',12)),'yyMM') ORDER BY APPLY_YYMM DESC),'31') JUN_MAGAM_DAY
        ,ISNULL((SELECT TOP 1 MAGAM_DAY FROM CM_M_CUST_MAGAM WHERE CUST_CODE=A.CUST_CODE AND APPLY_YYMM<='{ref_ym}' ORDER BY APPLY_YYMM DESC),'31') MAGAM_DAY
      FROM CM_M_CUST A )"""
    sql = f"""{magam}
...
def _receipt(dc, ref_ym):
  ...
  SELECT CUST_CODE,format(dateadd(MONTH,-1,convert(date,'{ref_ym}'+'01',12)),'yyMM') jun_yymm
    ,ISNULL((SELECT TOP 1 MAGAM_DAY FROM CM_M_CUST_MAGAM (nolock) WHERE CUST_CODE=A.CUST_CODE AND APPLY_YYMM<=format(dateadd(MONTH,-1,convert(date,'{ref_ym}'+'01',12)),'yyMM') ORDER BY APPLY_YYMM DESC),'31') JUN_MAGAM_DAY
...
"""
    ...
    _c3, nmrows = _rows("SELECT UPPER(item_code) c, item_desc d FROM pr_m_item")
...
def stockissue_view(from_ymd: str = Query(""), to_ymd: str = Query(""), pn: str = Query(""),
    ...
    """자재출고관리 라이브 조회(레거시 w_pu_stock_150, PU_T_STOCK_MAINT MAINT_TAG='B'). 서버 집계(건수·수량합)+페이징."""
    ...
    _c, agg = _rows(f"SELECT COUNT(*) cnt, ISNULL(SUM(a.maint_qty*-1),0) qty FROM pu_t_stock_maint a WHERE {WH}")
    ...
    sql = f"""
      SELECT a.maint_ymd ymd, a.maint_seq seq,
        ISNULL((SELECT gagong_proc_desc FROM pr_m_proc_gagong g WHERE g.gagong_proc_code=a.gagong_proc_code),a.gagong_proc_code) from_wh,
        a.item_code pn, ISNULL((SELECT item_desc FROM pr_m_item i WHERE i.item_code=a.item_code),'') pn_nm,
        CASE ISNULL(a.out_wh_gubun,'') WHEN '1' THEN '생산창고' WHEN '2' THEN '영업창고' ELSE '' END out_wh_nm,
        ISNULL((SELECT gagong_proc_desc FROM pr_m_proc_gagong g WHERE g.gagong_proc_code=a.to_gagong_proc_code),a.to_gagong_proc_code) to_wh,
      ...
      FROM pu_t_stock_maint a WHERE {WH}
      ORDER BY a.maint_ymd DESC, a.maint_seq ASC
      OFFSET {off} ROWS FETCH NEXT {sz} ROWS ONLY"""
    _c2, rows = _rows(sql)
    _c3, fw = _rows(f"SELECT DISTINCT a.gagong_proc_code code, ISNULL((SELECT gagong_proc_desc FROM pr_m_proc_gagong g WHERE g.gagong_proc_code=a.gagong_proc_code),a.gagong_proc_code) nm FROM pu_t_stock_maint a WHERE {PW} AND a.gagong_proc_code>'' ORDER BY 2")
    _c4, tw = _rows(f"SELECT DISTINCT a.to_gagong_proc_code code, ISNULL((SELECT gagong_proc_desc FROM pr_m_proc_gagong g WHERE g.gagong_proc_code=a.to_gagong_proc_code),a.to_gagong_proc_code) nm FROM pu_t_stock_maint a WHERE {PW} AND a.to_gagong_proc_code>'' ORDER BY 2")
...
def _prodinout(ym):
 ...
 SELECT a.TO_GAGONG_PROC_CODE part, UPPER(a.mat_code) mat, a.maint_ymd ymd, a.maint_qty*-1 inq,CAST(0 AS decimal(18,4)) outq,CAST(0 AS decimal(18,4)) etc,'생산창고입고' div, {CUST} tag
   FROM PU_T_STOCK_MAINT a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.maint_tag='B' AND ISNULL(a.out_wh_gubun,'1')='1' AND a.maint_qty<>0 AND {INSP} AND a.TO_GAGONG_PROC_CODE>''
 ...
 UNION ALL SELECT a.TO_GAGONG_PROC_CODE, UPPER(a.mat_code), a.maint_ymd, 0, a.maint_qty*-1,0,'자재창고반품',{CUST} FROM PU_T_STOCK_MAINT a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.maint_tag='T' AND ISNULL(a.out_wh_gubun,'3')='3' AND a.maint_qty<>0 AND a.TO_GAGONG_PROC_CODE>''
 UNION ALL SELECT a.TO_GAGONG_PROC_CODE, UPPER(a.mat_code), a.maint_ymd, 0, a.maint_qty,0,'가공부품이동',{CUST} FROM PU_T_STOCK_MAINT a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.maint_tag='C' AND a.maint_qty<>0 AND a.TO_GAGONG_PROC_CODE>''
 UNION ALL SELECT a.STOCK_PART_CODE, UPPER(a.item_code), a.prod_ymd, a.prod_qty,0,0,'SUB생산실적','' FROM pr_t_prod_dtl a WHERE a.prod_ymd>='{y01}' AND a.prod_ymd<='{y99}' AND a.STOCK_PART_CODE>'' AND NOT EXISTS(SELECT 1 FROM sa_t_stock_maint s WHERE s.maint_ymd=a.prod_ymd AND s.item_code=a.item_code AND s.in_part_code=a.stock_part_code)
 UNION ALL SELECT a.IN_PART_CODE, UPPER(a.item_code), a.maint_ymd, a.maint_qty,0,0,'생산실적',{CUST} FROM sa_t_stock_maint a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.IN_PART_CODE>''
 UNION ALL SELECT a.part_code, UPPER(a.mat_code), a.maint_ymd, a.maint_qty,0,0,'기초재고',{CUST} FROM PR_T_STOCK_MAINT_MAT a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.part_code>'' AND a.maint_tag='3' AND a.maint_qty<>0
 UNION ALL SELECT a.part_code, UPPER(a.mat_code), a.maint_ymd, 0,0,a.maint_qty,'재고조정',{CUST} FROM PR_T_STOCK_MAINT_MAT a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.part_code>'' AND a.maint_tag IN ('2','1') AND a.maint_qty<>0
 UNION ALL SELECT a.part_code, UPPER(a.mat_code), a.maint_ymd, 0, a.maint_qty*-1,0,'생산사용',{CUST} FROM PR_T_STOCK_MAINT_MAT a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.part_code>'' AND a.maint_tag='4' AND a.maint_qty<>0
"""
    BFT = f"'{y01}'"
    BF = f"""
...
"""
    _c1, uni = _rows("SELECT part_code part, UPPER(mat_code) mat, SUM(stock_qty) snap FROM pr_t_mat_stock_wh GROUP BY part_code, UPPER(mat_code)")
    ...
    _c5, sgrows = _rows("SELECT DETAIL_CODE cd, REPLACE(REPLACE(DETAIL_DESC,CHAR(13),''),CHAR(10),'') nm FROM CM_M_MASTER_DETAIL WHERE KIND_CODE='PR006'")
    _c6, pnrows = _rows("SELECT gagong_proc_code code, gagong_proc_desc nm FROM PR_M_PROC_GAGONG")
...
def _prodinvout(ym):
 ...
 UNION ALL SELECT UPPER(item_code), MAINT_QTY FROM sa_t_stock_maint WHERE MAINT_YMD>'250299' AND maint_ymd<'{y01}' AND maint_tag IN ('B','V','J','2','8','R')
 UNION ALL SELECT UPPER(item_code), MAINT_QTY FROM sa_t_stock_maint WHERE MAINT_YMD>'250299' AND maint_ymd<'{y01}' AND maint_tag='P' AND ISNULL(IN_PART_CODE,'')=''
 UNION ALL SELECT UPPER(mat_code), maint_qty*-1 FROM pu_t_stock_maint WHERE maint_ymd>'250299' AND maint_ymd<'{y01}' AND ISNULL(out_wh_gubun,'1')='2'
"""
    L1 = f"""
 SELECT UPPER(a.item_code) item, a.maint_ymd ymd, a.maint_qty inq, CAST(0 AS decimal(18,4)) outq, CAST(0 AS decimal(18,4)) etc, CASE a.maint_tag WHEN 'V' THEN '세트출하' WHEN 'P' THEN '생산완료' ELSE '입고' END div, {CUST} cust FROM sa_t_stock_maint a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.maint_tag IN ('B','V') AND a.maint_qty<>0
 UNION ALL SELECT UPPER(a.item_code), a.maint_ymd, a.maint_qty,0,0,'생산완료', {CUST} FROM sa_t_stock_maint a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.maint_tag='P' AND ISNULL(a.in_part_code,'')='' AND a.maint_qty<>0
 UNION ALL SELECT UPPER(a.mat_code), a.maint_ymd, a.maint_qty*-1,0,0,'자재창고에서입고', {CUST} FROM pu_t_stock_maint a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND ISNULL(a.out_wh_gubun,'1')='2'
 UNION ALL SELECT UPPER(a.item_code), a.maint_ymd, 0, a.maint_qty*-1,0, CASE a.maint_tag WHEN '8' THEN '무상공급' WHEN 'R' THEN '출하반품' ELSE '출하' END, {CUST} FROM sa_t_stock_maint a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.maint_tag IN ('J','8','R') AND a.maint_qty<>0
 UNION ALL SELECT UPPER(a.item_code), a.maint_ymd, 0,0, a.maint_qty*-1,'재고조정', {CUST} FROM sa_t_stock_maint a WHERE a.maint_ymd>='{y01}' AND a.maint_ymd<='{y99}' AND a.maint_tag='2' AND a.maint_qty<>0
"""
    _c1, uni = _rows("SELECT UPPER(item_code) item, SUM(stock_qty) snap FROM SA_T_ITEM_STOCK GROUP BY UPPER(item_code)")
    ...
    _c4, inforows = _rows("""SELECT UPPER(item_code) item, item_desc, (SELECT cust_desc FROM cm_m_cust c WHERE c.cust_code=i.in_cust_code) work_nm FROM pr_m_item i""")
...
SELECT t.mat cd, max(m.item_desc) nm, max(m.item_spec) spec, max(m.item_class) cls,
   ...
   (select top 1 item_cost from pr_m_item_cost where item_code=t.mat and cost_apply_ymd<='{t}' and cost_tag in ('S','E') order by cost_apply_ymd desc) cost,
...
UNION ALL SELECT a.to_gagong_proc_code,A.MAT_CODE,iif(a.maint_ymd<'{y01}',-A.MAINT_QTY,0),iif(a.maint_ymd<'{y01}',0,-A.MAINT_QTY),0,0 FROM PU_T_STOCK_MAINT A WHERE A.MAINT_YMD>'250299' and A.MAINT_YMD<='{y99}' AND a.maint_tag='B' AND isnull(a.out_wh_gubun,'1')='1'
...
UNION ALL SELECT a.to_gagong_proc_code,A.MAT_CODE,iif(a.MAINT_YMD<'{y01}',a.MAINT_QTY,0),0,iif(a.MAINT_YMD<'{y01}',0,-a.MAINT_QTY),0 FROM PU_T_STOCK_MAINT A WHERE A.MAINT_YMD>'250299' and A.MAINT_YMD<='{y99}' AND a.maint_tag='T' and isnull(a.out_wh_gubun,'3')='3'
UNION ALL SELECT a.to_gagong_proc_code,A.MAT_CODE,iif(a.MAINT_YMD<'{y01}',-a.MAINT_QTY,0),0,iif(a.MAINT_YMD<'{y01}',0,a.MAINT_QTY),0 FROM PU_T_STOCK_MAINT A WHERE A.MAINT_YMD>'250299' and A.MAINT_YMD<='{y99}' AND a.maint_tag='C'
UNION ALL SELECT A.stock_part_code,a.item_code,iif(a.prod_ymd<'{y01}',a.prod_qty,0),iif(a.prod_ymd<'{y01}',0,a.prod_qty),0,0 FROM pr_t_prod_dtl a WHERE A.prod_ymd>'250299' and A.prod_ymd<='{y99}' and a.stock_part_code>'' and not exists (select 1 from sa_t_stock_maint where maint_ymd=a.prod_ymd and item_code=a.item_code and in_part_code=a.stock_part_code)
UNION ALL SELECT A.IN_PART_CODE,a.item_code,iif(a.MAINT_YMD<'{y01}',a.MAINT_QTY,0),iif(a.MAINT_YMD<'{y01}',0,a.MAINT_QTY),0,0 FROM sa_t_stock_maint a WHERE A.maint_ymd>'250299' and A.MAINT_YMD<='{y99}' and a.in_part_code>''
UNION ALL SELECT A.PART_CODE,A.MAT_CODE,iif(a.MAINT_YMD<'{y01}',a.MAINT_QTY,0),iif(a.MAINT_YMD<'{y01}',0,a.MAINT_QTY),0,0 FROM PR_T_STOCK_MAINT_MAT A WHERE A.MAINT_YMD>'250299' and A.MAINT_YMD<='{y99}' AND A.MAINT_TAG='3'
UNION ALL SELECT A.PART_CODE,A.MAT_CODE,iif(a.MAINT_YMD<'{y01}',a.MAINT_QTY,0),0,0,iif(a.MAINT_YMD<'{y01}',0,a.MAINT_QTY) FROM PR_T_STOCK_MAINT_MAT A WHERE A.MAINT_YMD>'250299' and A.MAINT_YMD<='{y99}' AND A.MAINT_TAG in ('2','1')
UNION ALL SELECT A.PART_CODE,A.MAT_CODE,iif(a.MAINT_YMD<'{y01}',a.MAINT_QTY,0),0,iif(a.MAINT_YMD<'{y01}',0,-a.MAINT_QTY),0 FROM PR_T_STOCK_MAINT_MAT A JOIN PR_M_ITEM M ON A.MAT_CODE=M.ITEM_CODE WHERE A.MAINT_YMD>'250299' and A.MAINT_YMD<='{y99}' AND A.MAINT_TAG='4'
"""
    C2 = f"(select top 1 q.item_cost from pr_m_item_cost q where q.item_code=agg.mat and q.cost_tag='1' and q.cost_apply_ymd<='{y01}' and q.cust_code=case when pi.work_code='P2' then '2228' else pi.in_cust_code end order by q.cost_apply_ymd desc)"
    sql = f"""
;WITH agg AS (
...
FROM agg JOIN PR_M_ITEM pi ON pi.item_code=agg.mat
"""
    _c, rows = _rows(sql)
    return rows
...
def nxcompare(point: str = Query("MAT"), ym: str = Query(""), ymd: str = Query("")):
    ...
    if point == "MAT":
        ...
        d = _rows("SELECT MAX(STOCK_YMD) mx FROM PU_T_MONTH_STOCK_WH_DAILY WHERE cust_code='Z99990' AND STOCK_YMD<=?", key)
        mx = d[1][0]["mx"] if d[1] else None
        if mx:
            lv = _rows("SELECT ISNULL(SUM(stock_qty),0) t, COUNT(DISTINCT mat_code) c FROM PU_T_MONTH_STOCK_WH_DAILY WHERE cust_code='Z99990' AND STOCK_YMD=?", mx)[1]
...
def closestatus():
    ...
    rows = [
        {"domain": "자재(일마감)", "name": "자재재고(일)", "ctype": "일", "tbl": "PU_T_MONTH_STOCK_WH_DAILY",
         "last": mx("SELECT MAX(STOCK_YMD) FROM PU_T_MONTH_STOCK_WH_DAILY WHERE cust_code='Z99990'")},
        {"domain": "자재(월마감)", "name": "자재재고", "ctype": "월", "tbl": "PU_T_MONTH_STOCK_WH",
         "last": mx("SELECT MAX(STOCK_YYMM) FROM PU_T_MONTH_STOCK_WH WHERE cust_code='Z99990'")},
        {"domain": "생산(월마감)", "name": "생산 파트재고", "ctype": "월", "tbl": "PR_T_MONTH_STOCK_WH",
         "last": mx("SELECT MAX(STOCK_YYMM) FROM PR_T_MONTH_STOCK_WH")},
        {"domain": "영업제품(월마감)", "name": "영업 제품재고", "ctype": "월", "tbl": "SA_T_MONTH_STOCK",
         "last": mx("SELECT MAX(STOCK_YYMM) FROM SA_T_MONTH_STOCK")},
    ]
