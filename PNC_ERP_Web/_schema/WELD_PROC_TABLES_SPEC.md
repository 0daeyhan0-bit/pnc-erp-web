...
## 14. ★내부원가 UI 평면 재료표 전환 (2026-08-04, 구현완료)

- **내부원가 좌측 = LG BOM 수준 평면 재료표**(트리 아님, 대표 확정 [[newerp-sourcing-profile]]). 임의 SUB(SOCKET·20-1 등) **해체** → leaf 부품 평면. `/api/cost/nae` 역전개평면(naewon_nodes) 재사용 = `flatMat`(level>0 & mat>0, RAC=종류합산). SUB(haskids, mat=0)은 자동 제외.
- **편집모델**: 절삭부품(가공품, `make_type='1'` or `nproc>0` or silver — ★외주라도 공정 있으면 편집대상) 행 [✎]→**가공공정 팝업** / 제품(맨위) [✎ 조립공정]→**용접(관경별)·포장·체결 팝업**(node===item→isAssy) / 구매·부자재·사급(공정없음)=재료만(편집X).
- **용접/공정 총량 = 제품 레벨**(전 노드 합, 전공정 자체). SUB 구조는 조달 후보군에만.
- 구현: `screens.dev.js` `naeFlatMat()` 신설, 'proc' 뷰 좌측 `naeLevelTree`→`naeFlatMat` 교체(트리 제거). 이벤트=기존 `.nae-trow`(행클릭→우측조회)·`.nae-edit-btn`(✎→팝업, 제품버튼 data-node=item). `index.html ?v=260804j`. **프론트 전용(엔진/데이터/API 불변)** → 내부원가/실원가 == CS_M_ITEM_BOM 오라클 diff0 유지·43 baseline 불변.
- 검증(AJR30012009): SUB 2개(SOCKET·20-1) 해체→leaf 21행 평면 + 용접봉 2종(제품조립 0.0495=오라클). 가공품(MJU류, 외주 MJU66773002 포함 nproc>0)=편집대상, 구매/부자재(밸브·센서 등)=재료만. localhost만·미배포.

## 15. ★실원가 신규정의 — 외주 공정 유지 · 실원가 공정합=내부원가 (2026-08-04, ★구현완료·검증PASS)

> STEP A 개념검증(원가금액 불변 확인) → STEP B 구현 완료. 사용자 승인.

### 15.1 구현 결과 (STEP B)
- **`silwon_proc_grid`(nx_cost_engine.py) 수정**: 공정 **수량(wq) = 내부원가(proc_grid) 전노드**(외주 포함) · **원가(amt) = 자체노드(INNER)만**(현행 walk 보존). Σamt = silwon.gagong **불변**. `silwon()`/`gagong()`/`material` **원가엔진 로직 불변**.
- **프론트 변경 없음**: 실원가 탭 `drawSil`이 `procTable(silD.procs)` 렌더 → 엔진 변경 자동 반영(공정/용접/체결 view 수량=내부, 원가=실원가). ?v 불요. (백엔드 재기동 시 엔진 반영.)
- 백업 `scratchpad/nx_cost_engine.py.bak_stepB`.

### 15.2 ★게이트 재정의 (실원가)
- **기존**: "레거시 실원가 SP diff0"(외주 공정 drop) → **폐기**(SP EXEC 차단·불완전).
- **신규 게이트**: ①**실원가 공정/용접 수량 == 내부원가 수량**(silwon_proc_grid wq == proc_grid wq) ②**실원가 원가금액·손익 불변**(자체공임+외주매입가=현행) ③43 baseline 원가·손익 diff0.
- **내부원가 게이트 = 레거시 내부용(CS_M_ITEM_BOM) diff0** — 불변.

### 15.3 검증 (STEP B, before==after)
| 품목 | 실원가금액 | 손익 | 실공정wq(전) | 실공정wq(후)=내부 |
|---|---|---|---|---|
| AJR30012009 | 134929.57(불변) | 7759.43(불변) | 89 | **100(=내부100)** |
| AJR30012011 | 139741.03(불변) | 8703.97(불변) | 103 | **114(=내부114)** |
| AJR73327007 | 26669.39(불변) | -276.39(불변) | 56 | **96(=내부96)** |
| AJR30027714 | 231140.07(불변) | 3286.93(불변) | 123 | **200(=내부200)** |
- 원가금액·손익 **before==after 완전일치**(4/4), Σamt==silwon.gagong 정합, 용접공정(51/28) wq 실=내부(15=15), **43 baseline 원가 이탈 0**.
- ★수량만 통일(조달후보 비교용), 원가는 매입가 유지 → 외주분(재료232·공임316.95) 실원가 원가 **미가산**(손익 왜곡 방지).

---
### (구 설계案 원문 — 15.1로 구현됨)

- **외주(사급/매입)에서 사와도 용접/가공 공정을 빼지 않는다.** 전 공정(SUB 노드 포함) 매핑 유지. 추후 SUB 공정도 입력 가능(동일 매핑 구조).
- **내부원가 공정/용접 합 = 실원가 공정/용접 합**(동일·수량 보존). 외주분 수량도 살아있음.
- 외주/자체 구분·원가 귀속(자체 공임 vs 외주 매입가)은 **조달 후보군에서만**. 공정 수량은 어느 후보든 = 내부원가.

**레거시와의 차이**: 레거시 실원가 SP = 외주노드 공정 **drop**(`IIF(INNER_PROD_FLAG='1',WORK_QTY,0)`). 신규 = **drop 안 함, 공정 유지** → 신규 실원가는 레거시 실원가 SP와 공정 수량 달라짐(신규가 완전). **내부원가는 레거시 내부용 diff0 유지(불변)**.

**엔진 영향(구현 시)**: 현재 `NxCostEngine.silwon/gagong/material`은 `_inner_gagong`·`_expandable`(INNER_PROD 게이트)로 외주노드 공정·전개 drop. 신규는 실원가 공정합을 naewon(INNER 게이트 없음)과 동일하게 두고 **원가 귀속만 조달후보별**(자체=공임 / 외주=매입가).

**게이트 영향(★변경은 확인 후)**: 내부원가 게이트=레거시 내부용 diff0(유지). 실원가 게이트=현재 "레거시 실원가 SP diff0" → 신규 정의(공정합=내부원가+원가 조달귀속)로 **재정의 필요**. 재정의 전까지 실원가 엔진/게이트 **불변 유지**.
