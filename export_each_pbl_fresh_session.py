...
def export_single_pbl_fresh(pbl_path, all_pbl_files, base_out_dir):
    ...
    # Fresh session per PBL to avoid ORCA DLL memory corruption
    h_session = orca.PBORCA_SessionOpen()
    if not h_session:
        return 0, 0

    lib_arr_type = ctypes.c_wchar_p * len(all_pbl_files)
    lib_arr = lib_arr_type(*all_pbl_files)
    orca.PBORCA_SessionSetLibraryList(h_session, lib_arr, len(all_pbl_files))
...
def main():
    ...
    for i, pbl_path in enumerate(pbl_files):
        pbl_name = os.path.basename(pbl_path)
        try:
            succ, total = export_single_pbl_fresh(pbl_path, pbl_files, base_out_dir)
            total_objects += succ
    ...
    print("\n==========================================")
    print("FRESH SESSION 100% ALL PBL EXPORT COMPLETED!")
