...
def export_pbl_isolated_all(pbl_path, all_pbls, base_out_dir):
    ...
    for name in unique_names:
        for etype in range(11):
            ...
            if orca.PBORCA_LibraryEntryExport(h_session, pbl_path, name, etype, buf, 10 * 1024 * 1024) == 0:
...
def main():
    ...
    print("=== STARTING TOTAL 100% OBJECT EXTRACTION ===")
    total_exported = 0
    total_found = 0

    for i, pbl in enumerate(pbl_files, start=1):
        succ, found = export_pbl_isolated_all(pbl, pbl_files, base_out_dir)
        total_exported += succ
        total_found += found
        print("[{}/{}] {} -> Found: {}, Exported: {}".format(i, len(pbl_files), os.path.basename(pbl), found, succ))

    print("\nTOTAL COMPLETED!")
    print("Found Objects: {}, Exported Files: {}".format(total_found, total_exported))
