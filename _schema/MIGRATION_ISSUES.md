...
## ★컷오버 정리 대상 — 없는 업체(defunct vendor)·오래된 발주·음수재고 (2026-08-30 대표 확인)

**배경**: 자재매입현황(예상/실적) 구축 중 발견. **레거시엔 발주 개념이 없는데** `PARTNER_ERP.dbo.PU_T_PURCHASE_DTL`에 **아주 오래된(2023년) 미입고잔량**이 남아, **거래처마스터(nx.cust)에 없는 업체**(제이앤아이 sub·LGE11·SW####·SWP###·P진티엔·P#### 등)에 붙어 있다. 대표: **"이런 업체들은 없는 업체·컷오버때 다 없앨 품목들"**. 음수 재고도 동일(컷오버 정리).

**실측(2026-08-30, 오늘~+4주 기준)**:
- PU_T_PURCHASE_DTL 미입고잔량 vendor 67개 중 **48개가 nx.cust 미등재**(=없는 업체). 발주일 전부 **2023년**(예: SW2633 잔량187,111 발주230125·SW5001 173,273·SW2413 118,551·P진티엔 20,007). 2년+ 미입고=사실상 죽은 발주.
- plan_mat_source(소요) vendor는 111개 중 1개(`'0'`, 외주가공)만 미등재 → 소요측은 깨끗.
- 음수 재고(nx.mat_stock_daily) 일부(LG −533·MTS −463 등)도 컷오버 정리 대상.

**컷오버 시 적용(purge 대상)**:
1. **PU_T_PURCHASE_DTL 미입고잔량 중 거래처마스터(nx.cust) 미등재 업체 + 2년+ 오래된(예: 2023) 발주** = 삭제/마감처리(IN_FINISH_FLAG 마감 or 폐기). 발주 개념 자체가 신규 ERP 수동/자동발주로 대체되므로 레거시 stale 발주는 이관하지 않음.
2. **없는 업체 vendor 코드**(SW####/SWP###/LGE11/P진티엔/P#### 등)가 붙은 품목·조달프로파일 정리(거래처 identity 통합 or 폐기).
3. **음수 재고** 정리(기초 재정합).

**임시 조치(컷오버 전, 이미 반영)**: `matexpect.py` 예상 탭 기발주에서 **거래처마스터(nx.cust) 미등재 vendor 제외**(컷오버 삭제대상 stale 발주 노이즈 차단). 기발주 총 49,635→28,974, 미분류 39→16행. 컷오버로 원천 정리되면 이 필터는 무해(대상 0). 정본 = `MAT_EXPECTED_PURCHASE_DESIGN §14` + [[newerp-matexpect-initiative]].
