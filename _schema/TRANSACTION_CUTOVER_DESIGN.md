...
- **결론**: 트랜잭션 읽기도 **마스터와 동일하게 nx 미러로 repoint 가능**(before/after 출력 hash 일치 검증). = 백엔드 레거시 직독 0 달성.
- **단 하나의 트레이드오프 = 실시간성**: 미러는 "마지막 델타싱크 시점" 상태. 일마감/일계획 용도면 일1회 델타로 충분. 인트라데이 실시간 필요분(가공진척·당일완료)은 델타 주기 상향(예 15분) or 해당 읽기만 라이브 잔류. → **읽기별 실시간요구 등급화** 후 repoint.
