...
### 6-1. ★엔진CS vs R01PR 갭 (r_bom_compare.py) — 통일의 핵심 난제
- **원가엔진 nx.bom_line = CS(원가용) 기반 / R01 route = PR(생산 실사용) 기반** → 리프·코드 표현 다름. 예 AJR77263008: 엔진=관+은납재(BCUP) 따로 / R01=`MJU..+용접링` 결합. AJR75563402 엔진10리프 vs R01 2리프.
- 둘 다 각자 레거시 재현(엔진=실원가 CS diff0 5722.2 / R01=재료비 PR diff0)이나 **서로 다름** = "BOM 3중분리" 실체. **그냥 교체하면 엔진 원가 깨짐.**
