...
### 4-D. 원소재 backflush 재설계 — 보류·기록만 (대표 지시 2026-09-08)
실제 흐름 추적으로 매칭 접근(STD/LG_AP)이 현실과 어긋남이 드러나, **원소재 backflush 설계를 실제 소비 데이터 기반으로 재고민**하기로. 지금은 **기록만**, 다른 BOM TABLE 정리 먼저.
- **A방식(LG_AP 우선) 검증 결과 부적합**: OLD(STD)vs NEW(A) 전수비교 — **차감대상 코드변경 1,483건(70%)**. LG_AP가 STD의 7072AR9374x(Tube,Raw·재고보유) 대신 **MJU계열 원소재(재고 없을 공산)** 를 지목 → 빈 코드 차감 위험. ⟹ A 채택 보류.
- **★실제 원소재 소비 추적(stock_ledger 2501~·동 sgroup210 169코드)**:
  · 입고 tag9 **+220,527kg** · 협력사반출 tag5 **−171,340**(제외) · 기초 OB.
  · **우리 소비 = tag B −67,488kg**, REMARKS="1롤·2롤·196본·121본…" = **본/롤 단위 수기 출고**(재고관리자가 절삭 투입 raw를 수기 출고). 자동 중량 backflush 아님.
- ⟹ 현실=본/롤 수기. 매칭 실패도 그 탓(원소재 소비를 가공품 중량으로 안 잡고 롤/본으로 잡음).
- **설계 미결(2안)**: (가)절삭 실적 자동 중량차감으로 전환(수기 대체) / (나)본/롤 유지+절삭 예상소비 대사(로스 리포트). 다음 착수 시 tag B 출고↔절삭실적 연결 추가 추적 후 결정.
- 현재 코드상태(feat·미배포): procbc 중량소스=bom_flat 수정·엔진 `dong_unit_weight`/`std_rawmat_of`/`rawmat_for_cut` 추가됨. **원소재 차감 로직 자체는 미변경(STD 5키 inline 유지)·배포 안 함**. 설계 확정 후 반영.
