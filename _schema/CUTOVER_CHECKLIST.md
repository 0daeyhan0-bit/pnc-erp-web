...
### ✅ 8. BOM `except_flag` 재싱크 — **재싱크할 게 없다. 드리프트 0.**

```
nx.bom_line ⋈ nx.bom_header  vs  레거시 PR_M_ITEM_BOM   (키 = 부모+자식)
   nx 키 37,547 · 레거시 키 42,457 · 공통 37,281
   ★플래그 불일치 0건   (nx=0/레거시=1: 0 · nx=1/레거시=0: 0)
```

**겉으로 보이는 총수 차이는 전부 설명된다.**
| | |
|---|---|
| nx `except_flag=1` | 2,062 |
| 레거시 `EXCEPT_FLAG=1` | 6,984 |
| 레거시에만 있는 키 | 5,176 (그중 except=1 이 **5,154**) |
| 그 자식이 **용접봉(`RAC*`)** | **5,120 = 98.9%** |

용접봉은 `nx.bom_line` 자식으로 두지 않고 **`nx.proc_weld` 로 뺐다**(`DO_NOT_USE §10`).
누락이 아니라 **설계**다. `2,062 + 5,154 ≒ 6,984` 로 수가 맞아떨어진다.
⟹ **항목 종료.** 재싱크·재편성 불필요.

### ◐ 9. 우리 편성 `nx.plan_*` vs 레거시 — **갭이 크게 줄었고, 성격도 대부분 규명**

체크리스트가 적어 둔 갭은 `85,773 vs 110,638`(≈24,865)였다. **지금은 다르다.**
```
같은 기간(260828~) 기준
   nx.plan_part_mat 79,849  vs  레거시 PR_T_PLAN_PART_MAT 83,127   차 −3,278
```

#### 3,278행의 분해
| 성격 | 건수 | 판정 |
|---|---|---|
| **날짜만 하루 다름** | 레거시쪽 1,251 · nx쪽 1,207 | 소요일자 계산 차이(당김/근무일) |
| **용접봉 `240`·`910`** | **1,377** | **의도된 것** — 8번과 같은 이유(`proc_weld` 분리) |
| 그 외(`230`·`130`·`310`·`120` 등) | 약 1,600 | ☐ 추가 확인 |
| 수량 다른 키 | **410 (0.55%)** | ☐ 추가 확인 |

**날짜를 빼고 (제번+자재)로만 보면**
```
공통 74,496 · nx만 355 · 레거시만 3,011 · 수량 다른 것 410 (0.55%)
```
⟹ 대부분 **같은 소요를 다른 날짜에 잡은 것**이고, 진짜 없는 건 훨씬 적다.

#### 남은 확인 (컷오버 필수는 아님)
1. **날짜 하루 차이**의 원인 — 당김근무일/캘린더 규칙 차이로 보인다([[newerp-coop-plan-delivery-formulas]] 축).
2. `230`·`130`·`310` 등 **약 1,600건**이 왜 nx 에 없는지.
3. 수량 다른 **410건(0.55%)**.

> **판정**: 24,865 → 3,278 로 줄었고 그중 절반 가까이가 **설계상 의도(용접봉)** 이거나 **날짜 이동**이다.
> 컷오버를 막을 수준은 아니다. 다만 **"의도한 차이"로 확정하려면 위 3가지를 마저 봐야 한다.**

---

## 9번 심화 — 날짜 차이의 정체 (2026-08-29)

### 확정된 것

**① 주말 처리는 양쪽이 같다.** 달력일로 흩어지던 차이가 **근무일로 환산하면 ±1일로 수렴**한다.
```
달력일 차이 : +1일 755 · +3일 197 · −3일 188 · −2일 36 · −1일 31
근무일 차이 : +1일 952 · −1일 219 · −2일 36        ← 주말 건너뛰면 정리됨
```
`+3일`은 전부 **금요일→월요일**이었다. 즉 주말 규칙 차이가 아니다.

**② `+1일` 그룹(952건)은 완벽히 균질하다.**
| | |
|---|---|
| `bom_level` | **전부 0** (최상위) |
| 작업장 | `P1`(사내용접) **415** · `2148`(대원산업) **415** — 정확히 동수 |
| 제번 | 424개 · 그중 **415개가 P1+2148 쌍**을 이룸 |
| 자재 | **자재 = 상위품목**(자기소요) 424건 · `-SUB` 415건 |

⟹ 세트가 두 작업장으로 갈라지는 **레벨0 자기소요 행**이다. 무작위 오류가 아니라 **구조적**이다.

**③ `−1일`(219) · `−2일`(36) 은 성격이 다르다** — 레벨 0/1/2/3 에 흩어지고 작업장도 제각각.

### ★내가 틀린 것 — 성급한 일반화 (정정)
`+1일` 952건에서 **nx 의 `part_plan_ymd` 가 레거시 `PLAN_YMD` 와 100% 일치**하는 걸 보고
*"대응 컬럼을 잘못 골랐다, 데이터는 맞다"* 고 결론냈다. **틀렸다.**

전체를 `part_plan_ymd ↔ PART_PLAN_YMD` 로 다시 재니 **오히려 나빠진다.**
| 대응 | 공통 | nx만 | 레거시만 | 수량다름 |
|---|---|---|---|---|
| `plan_ymd ↔ PLAN_YMD` | **73,361** | 1,562 | 4,262 | 300 |
| `part_plan_ymd ↔ PART_PLAN_YMD` | 64,417 | 11,325 | 14,267 | 360 |

⟹ **원래 쓰던 `plan_ymd` 대응이 맞다.** 952건에서 컬럼이 맞아떨어진 건 그 부분집합의 성질이지
전체 규칙이 아니었다. **부분집합 결과를 전체로 일반화한 실수.**

### 현재 확정 수치 (`plan_ymd` 기준, 260828~)
```
nx 79,849 · 레거시 83,127 · 차 −3,278
   공통 73,361 · nx만 1,562 · 레거시만 4,262 · 수량 다른 키 300 (0.41%)
   근무일 ±1일 이동으로 설명되는 것 : 1,207(nx쪽) + 1,251(레거시쪽)
   용접봉(240·910) 설계상 분리      : 1,377
```

### ☐ 남은 숙제
1. **`+1일` 952건** — 레벨0·P1/2148 쌍·자기소요라는 **구조**는 잡았으나 **왜 하루 늦는지**는 미규명.
   `part_plan_ymd`(레거시 `PLAN_YMD` 와 일치) → `plan_ymd` 로 갈 때 하루를 더하는 규칙이 있는 듯.
   소요 전개 STEP(당김근무일) 어디서 붙는지 코드 추적 필요.
2. `−1일` 219 · `−2일` 36 — 성격 미상.
3. 용접봉 외 나머지(`230`·`130`·`310`·`120`) 약 1,600건.
4. 수량 다른 300건(0.41%).

> **판정 변화 없음**: 컷오버를 막을 사유는 아니다(0.41% 수량차, 나머지는 날짜 이동/설계).
> 다만 **"의도한 차이"로 확정하려면 위 4가지가 남았다.** 지금은 "성격은 잡았고 원인은 일부 미규명" 이다.

---

## ✅ 9번 결론 — **내 비교 자체가 잘못됐다** (2026-08-29)

### 대표 설명으로 방향이 잡혔다
> *"기존 레거시는 계획당김 기능이 있었는데 신규는 잘 안 됐고, 한대윤 차장이 개발해 이제 정상 작동한다."*

기록을 찾아보니 **이미 진단·수정·검증이 끝난 사안**이었다.

### 한대윤 차장(`Dyun`) 작업 — 2026-08-27
| 커밋 | 내용 |
|---|---|
| `cc6f91d` | **당김 후 소요일시(`part_plan_ymd`/`hm`) 전달** — 원본일자만 갖던 것 교정 |
| `4f0da52` | **당일 클램프 적용** — `part_plan_ymd` 81.56% → **99.27%** |

`cc6f91d` 원인 분석이 내가 세운 가설과 **정확히 같다**:
> 레거시 `PR_T_PLAN_PART_MAT` 은 날짜를 **2벌** 갖는다.
>   `PLAN_YMD` = 상위 계획(**당김 전**)
>   `PART_PLAN_YMD` = **당김 후 소요일시** ← 자재는 이걸 본다
> 웹 `nx.plan_part_mat` 에는 `plan_ymd` 하나뿐이라 당김이 유실됐다.

그리고 **검증 수치도 이미 있다**:
```
part_plan_ymd 81.56% → 99.27%
★기준일 동일 행만 : part_plan_ymd 100.00% (89,414/89,414)
```

### ★내 3,278행 차이의 진짜 원인 — 편성 시점이 다르다
```
nx.plan_part_mat          기준일 260828   ← 어제 편성
레거시 PR_T_PLAN_PART_MAT   기준일 260827   ← 그저께 편성
```
**하루 차이 나는 서로 다른 편성 결과**를 비교했다. 날짜가 어긋나는 게 당연하다.

`4f0da52` 가 그 민감도를 이미 실측해 뒀다:
```
기준일 260826 → 80.78%
기준일 260827 → 100.00%
기준일 260828 → 76.86%
```
**기준일이 하루만 달라도 80% / 100% / 77% 로 출렁인다.**
⟹ 내가 본 `−3,278행`·`+1일 952건`은 **당김 결함이 아니라 기준일 불일치**다.

### 교훈 — 같은 데이터로 비교하라
자기점검 목록의 **"같은 데이터 비교"** 를 또 어겼다.
계획처럼 **매일 재편성되는 산출물**은 **기준일(편성 시작일)이 같은 것끼리** 비교해야 한다.
행수·날짜만 보고 "갭"이라 부르면 **매일 다른 답이 나온다.**

> 그리고 **먼저 기록을 읽었으면** 이 추적 전체가 불필요했다.
> `cc6f91d`/`4f0da52` 커밋 본문에 원인·조치·검증이 다 적혀 있었다.
> ([[feedback-read-canon-before-investigating]] 재발)

### ⟹ 항목 판정
**9번 = 종료.** 우리 편성은 레거시와 **기준일 동일 조건에서 `part_plan_ymd` 100.00%** 로 검증돼 있다.
남아 있던 "☐ 숙제 4가지" 중 **1·2번(날짜 ±1일)은 이 건으로 해소**된다.
| 남은 것 | 상태 |
|---|---|
| 용접봉 외 나머지(`230`·`130`·`310`·`120`) 약 1,600 | 기준일 불일치 영향 포함 — **같은 기준일로 재측정 후 판단** |
| 수량 다른 300건(0.41%) | 동일 — 재측정 대상 |

**컷오버 당일 recon 은 반드시 같은 기준일로 편성한 뒤 비교할 것.**

---

## 10번 이중입력 차단 — 실태 측정 + 차단 방법 (2026-08-29)

### ★결론 먼저 — **지금 이중입력은 없다**
걱정하던 "레거시·웹 양쪽 입력"이 **실제로는 벌어지지 않고 있다.**

**최근 30일 거래 테이블 입력 출처 (INSERT_WINDOW 실측)**
| 테이블 | 30일 건수 | 출처 |
|---|---|---|
| `PU_T_STOCK_MAINT` | 38,422 | `w_pr_input_460_new` 25,692 · `w_pr_input_135` 6,144 … **전부 레거시 화면** |
| `SA_T_STOCK_MAINT` | 10,986 | `w_pr_input_040` 7,022 … **전부 레거시** |
| `PR_T_STOCK_MAINT_MAT` | 28,060 | `dw_t2` 17,389 … **전부 레거시** |
| `PU_T_READY_STOCK_MAINT` | 27,102 | `w_pr_input_520` 16,190 … **전부 레거시** |

**웹 입력 흔적 0건.**

**웹 쓰기는 어디에 남나** — `nx.stock_ledger` 최근 30일 **593건**뿐이고 그마저 대부분 테스트다.
```
web 569 · 테스트1(전권) 12 · 테스트3(생산) 6 · 웹사용자 6
재고점/태그: MAT tag9 507 · MAT tag2 31 · RDY K1 18 · ASY J 7 …
```
`stock_ledger` 전체 172,450행 중 대부분은 **레거시 화면에서 이관된 분**이다
(`INSERT_WINDOW` = `w_pr_input_460_new` 114,499 · `w_pr_input_135` 24,520 …).

⟹ **직원은 아직 레거시로만 업무하고, 웹은 테스트 중이다.**

### 그래서 10번의 성격이 바뀐다
| 시점 | 상태 |
|---|---|
| 지금 | 이중입력 **없음** — 레거시 단일 입력 |
| 컷오버 후 | 웹 단일 입력 — 레거시를 막으면 끝 |
| **위험 구간** | **컷오버 당일, 레거시를 막기 전까지의 틈** |

즉 *"지금 뭘 고칠 것"* 이 아니라 **"컷오버 때 레거시를 확실히 막는 절차"** 다.

### 차단 방법 — DB 계정 하나로 끝난다
```
현재 접속 계정 : ilshin
PARTNER_ERP 사용자 : ['ilshin']  ← 이 계정 하나뿐
```
레거시 PowerBuilder ERP 는 **`ilshin` 계정 하나로 DB 에 붙는다.**
⟹ 그 계정의 **쓰기 권한만 회수**하면 레거시 전체가 즉시 읽기전용이 된다.
화면을 하나씩 막을 필요가 없다.

```sql
-- 컷오버 밤, 레거시 차단 (라이브 PARTNER_ERP 에서)
ALTER ROLE db_datawriter  DROP MEMBER ilshin;
DENY INSERT, UPDATE, DELETE TO ilshin;
-- 롤백 시
ALTER ROLE db_datawriter  ADD MEMBER ilshin;
```
> ⚠ **우리 백엔드도 같은 계정을 쓴다.** 우리는 `PARTNER_ERP_TEST3`(nx)에 쓰므로
> **라이브 DB 에서만** 회수해야 한다. `TEST3` 권한은 건드리지 말 것.
> 회수 전 반드시 확인: 웹 백엔드가 라이브에 쓰는 코드가 0인지(`DO_NOT_USE §5` — 라이브는 읽기전용).

### 컷오버 절차 반영
```
① cutover_rollback.py --snapshot          되돌림 기준점
② price_item_delta_sync.py --commit       단가 마지막 반영
③ 레거시 쓰기 권한 회수 (위 SQL)          ★이중입력 차단 = 10번
④ cutover_mark.py --set --commit          레거시 기준 sync 전부 정지
⑤ 일괄 flip · recon(★같은 기준일로)
```
③ 과 ④ 순서 주의 — **레거시를 먼저 막고**, 그 다음 sync 를 정지한다.
반대로 하면 막기 전 마지막 입력분이 정본에 안 들어온다.
