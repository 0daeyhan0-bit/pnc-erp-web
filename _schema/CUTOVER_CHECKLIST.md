...
**① 수출단가 스코프 누락 12행 (tag E · 거래처 2325 = Qingdao Impulse)**
클린 `price_item` 의 TAGE 는 거래처가 **1010/1020/1030 뿐**이다 — LG 만 담았다.
미러에는 2325(청도) 12행이 더 있는데 **클린에는 그 품목의 TAGE 가 아예 0행**이다.
⟹ 지금 repoint 하면 이 12품목의 **수출단가가 사라진다.**
```
3H01366A/B/C/D · 4H00189A · 4H01690A/B/C · FAD3105190 1~4   적용 241114
예: FAD31051904  0.93763 USD   (3H01366B 만 KRW — 통화 혼재, 확인 필요)
```

**② 매입단가 10행 (tag 1 · 거래처 1150/2011/2016/2272/2306)**
`-은납체결` `-SUB` `-SUB2` `-19-1` 같은 **접미사 품번**이 많다.
접미사 품번은 `bom_merge_map`/SUB 정규화에서 다뤄온 축이므로
이관 스크립트가 정규화 과정에서 떨군 것으로 보인다 — **건별 확인 필요**.
```
AJR74618301-SUB(1150) · AJR32883908-은납체결(2011,2272) · AJR73982802-은납체결(2011,2272)
AJR75645201-SUB2(2011,2272) · AJR76462903-은납(2016) · AJR30105801-19-1(2306) · AJR33796513-19-1(2306)
```

### 순서 (이 순서를 지켜야 한다)
1. **갭 22행 해소 먼저.** 클린을 정본으로 만든 뒤에 repoint 한다.
   순서를 뒤집으면 12품목 수출단가가 **조용히 0/None** 이 된다.
   → 단가 데이터 보강이므로 **대표 승인 후** 실행.
2. 읽기 22곳 repoint (`sourcing`·`price`·`coopquote`·`coopquote2`·`sales`·`dtrade`).
3. `pricemgmt.py` 쓰기 8곳을 클린으로. 화면이 **클린에 직접 쓰게** 한다(미러 이중기록 금지).
4. 화면 값 대조 — repoint 전후 동일함을 실측.
