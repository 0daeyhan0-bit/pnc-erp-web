...
| # | 항목 | 상태 |
|---|---|---|
| G-1 | **`_mat_avail` 을 `mat_stock_daily` → 실시간(확정 스냅샷 + 이후 전표)으로 승격** | ☐ **필수** |
| G-2 | 승격 후 게이트 오판 재측정(현재 133품목 오통과) | ☐ |
| G-3 | `mat_stock_daily` 은퇴 + 빌더(`matclose_movavg_build.py`) 폐기 | ☐ |
| G-4 | 게이트가 **수불장·마감과 같은 엔진**을 부르는지 확인(별도 SQL 금지 — 값이 갈린다) | ☐ |
...
- 실측: 빌더가 **8/25 에서 멈춤** → **133품목이 "재고 있음" 으로 오판**되어 음수재고를 통과시킨다.
  `5210A22409A` — 게이트 2,241 vs 실제 −2,659.

### 실시간 정본 실측 (2026-08-28)
```
확정 스냅샷(2607) + 8월 전표 = 3,694품목 · 산출 0.75초   ← 게이트에 쓰기 충분
음수 품목: 실시간 40 vs 구방식 139        ← 구방식이 재고를 과소평가하고 있었다
구방식과 값이 다른 품목 1,432
```

### ★주의 — 게이트용 SQL 을 새로 짜지 말 것
검산용으로 별도 SQL 을 짰더니 수불장과 **56건이 갈렸다**(scope·전개순서 차이).
**같은 값은 한 곳에서 계산한다**(§21). 게이트도 `_mat_ledger`/`_mv_*` 같은 **엔진을 호출**해야 한다.
