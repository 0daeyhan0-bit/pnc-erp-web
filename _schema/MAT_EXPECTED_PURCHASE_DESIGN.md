...
- **★해외 장리드 PSI·선발주(사용자 확정 2026-08-26)**: 해외 거래처(태국 F&T 등)는 리드타임이 길어 **PSI 관리 + 선발주**. horizon 2계층:
  ...
  - **4주 초과** = **LG 물동계획**(내시/forecast) 참고 → 선발주. ⚠**LG 물동계획 데이터 피드가 현 시스템에 없음**(물동/forecast 테이블 부재) = 데이터 갭(§10). 확보 전까지는 이미 나간 **선발주=미착(발주잔량)** 으로 파악.
  - **미착(기발주/선발주) = `PU_T_PURCHASE_DTL` 미입고 잔량**(`PUR_QTY−IN_QTY−CANCEL_QTY`, `DLVY_YMD`=예정입고). manorder가 이미 사용(재사용). 순소요에서 차감 + **미착 컬럼으로 표시**(선발주 가시화).
...
- 유상사급 부품·동은 **LG가 지급**(우리가 구매하는 게 아님)이지만, **자재예상매입에 3분류의 한 축으로 포함**한다. 단 컬럼 의미(입고실적 소스)가 분류마다 다름:
  - **원소재(직구매)·그외** → 입고실적 = **우리 매입입고**(`PU_T_STOCK_MAINT` 9/S/C/G/H + `_C` P).
  - **사급** → 입고실적 = **LG 지급분**(OSP 사급입고 = lgsagub 3-way ②, LG전산 기준). 필요시 우리ERP 확정입고(①)와 병기.
- 사급 소요: 부품=리시빙×BOM(`lgsagub.recvcompare_parts`, OSP목록 정지·사급단가=OSP청구가) · 동=리시빙×원단위중량(`lgsagub.recvcompare`, gubun1로 사급/직거래·동단가=mat_cost). 재사용.
- **필요수량(사급) = 사급 순소요 vs LG지급** → "LG가 소요만큼 충분히 줬나 / 부족·과다" 판단(사급정합 축). 발주(우리 구매)와 의미 구분해 표시.
...
| **매입실적**(실제 구매입고, 수량/금액) | 원소재·그외 = **자재창고입고(tag 9)** + **수입(도입-수입, `_C` DIVISION='P')**. 사급 = 세트입고(tag S)/LG지급(OSP). ★**내부 가공이동(C가공이동입고·G축관입고·H가공입고)은 구매 아님=제외**(금액0). CUST_CODE별. | 수입 외화→KRW=ROUND(amt×rate,0,1) 버림 |
...
3. **영업축 예상소요**: **예상 소요 = 4주 생산계획**(사용자 확정)이 공통 기준. 영업축은 **실적구간(출하실적×BOM)** 에서만 생산축과 다름. (영업축 예상도 생산계획 사용 or 별도? — 확인)
4. **안전재고·커버 N일 파라미터**: 리드타임 외 안전재고 별도? 커버일수?
5. **금액 단가**: 재고=이동평균 / 소요·필요=매입단가(price_item) 로 확정?
6. ~~미착(입고예정)~~ **[확정 2026-08-26]**: **미착 = `PU_T_PURCHASE_DTL` 미입고 발주잔량**(`PUR_QTY−IN_QTY−CANCEL_QTY`, 수동발주 선발주 포함) → 순소요 차감 + **미착 컬럼**(선발주 가시화). manorder 재사용.
7. **★LG 물동계획 데이터 갭(신규 과제)**: 해외 장리드 선발주용 **4주 초과 LG 물동계획(내시) 피드가 현 시스템에 없음**(생산계획·주문 모두 ~4주까지만). → LG PU-SCS 물동/demand 다운로드 or 수동업로드 신설 필요. 확보 전엔 4주 horizon + 미착(선발주)으로 운영.
