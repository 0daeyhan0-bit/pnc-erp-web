...
- **사급 = LG전자 → PNC 수령 사급부품(우리가 LG에서 받아오는 유상사급부품, CUST_TYPE=1)**. 우리가 구매하는 게 아니라 **LG가 지급**(≠ PNC→협력사 불출 사급). **자재예상매입에 3분류의 한 축으로 포함**하되, 컬럼 의미(입고실적 소스)가 다름:
...
| **필요수량**(순소요, 수량/금액) | `max(0, 총소요 + 상시보유 − 현재고 − 미착)` (미착=발주잔량, 확정발주 포함) | MRP 넷팅(§7 autoorder 재사용) |
