...
- **★발주 정책(사용자 확정 2026-08-26)**:
  - **협력사(절삭협력 등 단리드)** = **발주 안 함**. 계획 공유(협력사계획현황410/명세서420) → 협력사가 스스로 납품. (매입 아님·가공비 축)
  - **도입·원자재 등 장리드** = **수동발주(선발주)**. 수동발주를 장리드 조달용으로 정비. ← 컷오버 후 이 발주 저장분이 **미착** 소스.
  - **자동발주 = 도입 안 함**(불필요). 필요수량(순소요) 계산은 적정성 판단용일 뿐, 자동 발주 생성 아님.
...
| **필요수량**(순 매입 필요량, 수량/금액) | **`max(0, 월 총소요 + 상시보유 − 기초재고)`** ★기초재고(월초) 기준. **미착은 컷오버 후 `− 미착` 추가**. | 매입 적정성 기준 |
...
- **`autoorder.py`** = 넷팅 **계산 로직만 참고**(순소요 산식). ★**자동발주 기능은 도입 안 함**(사용자 확정 08-26) — 필요수량은 적정성 판단용, 자동 발주생성 아님. 발주는 장리드만 **수동발주**(협력사는 계획공유). 같은 **소요/배분 정본** 소비(수량 일관).
...
6. ~~미착~~ **[확정 2026-08-26]**: **미착은 컷오버 이후 도입.** 현재는 필요수량에서 제외(`max(0, 월소요+상시보유−기초재고)`). **발주정책**: 협력사=계획공유(발주X)·**도입/원자재 장리드=수동발주(선발주)**·**자동발주 없음**. 컷오버 후 수동발주 저장분이 미착 소스.
