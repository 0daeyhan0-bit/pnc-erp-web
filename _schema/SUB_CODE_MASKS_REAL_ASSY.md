...
### 7-1. ★확정 설계 (2026-08-15, 사용자 결정)

**SUB 정체성 = 구성품 + 전체 공정(시그니처 기반). 위치·route 무관, 1개 코드 전역 재사용.**

1. **route(Rxx)는 코드에 포함 — "출생 맥락"으로 고정** (★2026-08-15 재정정: 초기에 "R 제거"라 했으나 오류. 생성 시점엔 공용 여부를 모르고 그 SUB는 특정 ASSY의 특정 route에서 태어나므로, 출생 route를 코드에 박제). R = 최초 생성 route(고정, 사용 route 아님) → 공유돼도 안 애매 + 생성-시-모름 문제 해소.
2. **SUB 코드 = `{첫작업ASSY}_R{첫작업route}_S{nn}`** — 그 SUB를 **처음 작업(최초 등록)한 ASSY+route** 를 출생라벨로 박제. 어디(다른 ASSY/route) 쓰이든 이 코드 그대로 재사용. 구성 다른 route변형은 시그니처가 자동 분리(다른 코드).
3. **시그니처 = 구성품 + 용접** (★2026-08-15 재확정: "전체 공정"에서 **구성품+용접만**으로 변경). 근거: 용접=SUB 물리적 정체성(접합), 포장·검사=마감(정체성 아님), 절삭=부품공정(ASSY는 절삭 안 함→용접만 보면 자동배제). 구조차(굽힘·치수)는 구성품에 이미 반영. **→ 현 `_sub_signature`(children+proc_weld) 그대로, 로직 변경 불필요.**
   - **판정**: 용접 다름 or 구성품 다름 → 다른 SUB. **포장·검사 등만 다름 → 같은 SUB(1코드)**, 차이는 라우팅 축에서 표현(원가엔 nx.routing으로 정상 반영, SUB 정체성에만 미포함).
4. **공용(shared) flag 신설** — 여러 ASSY가 쓰는 SUB에 **공용 플래그**(참조 ASSY 수>1). 일반 SUB와 **다르게 관리**(변경 영향범위·승인 등)하기 위함. (참고: sub_alias에 is_shared·n_ref_base 기존 존재 → sub_registry에도 공용 플래그/참조수 반영)
5. **실제 제품**(자기 품번·도면)엔 SUB코드 금지(§3, 수정 완료). **원본 자도번 코드 병기**(직원 대조).

**정합:** "route마다 SUB 구성 다름"(사용자) = 구성 다름 → 시그니처 다름 → 코드 자동 분리. "정체성=구성+공정" = route 무관. 둘이 시그니처로 일관 통합.

**구현 범위:** 표시(대표ASSY_S{nn})·시그니처 확장·공용 플래그·영속 코드 = **마이그레이션 정본화 과제**. 현재는 §3 실제제품 코드보존까지만 적용, 나머지는 미러 유지.
