...
3. **시그니처 = 구성품 + 전체 공정** — 공정은 **용접·포장 등 모든 공정이 동일**해야 같은 SUB. ★현재 `nx.sub_registry.sig`는 children+proc_weld(용접봉)만 반영 → **전체 공정(routing)까지 포함하도록 확장 필요.** (확장 시 dedup 결과 변함: 같은 구성+용접이라도 포장 등 다르면 분리 → distinct SUB 수 증가 가능)
