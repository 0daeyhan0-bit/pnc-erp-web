...
1. **route(Rxx)는 코드에서 제외** — 구성 다르면 시그니처가 자동 분리하므로 불필요. route는 "제품이 경로별로 어떤 SUB를 쓰는가"(BOM 구조 축)로만 표현.
2. **SUB 코드 = `{대표ASSY}_S{nn}`** — 대표ASSY = **그 SUB를 처음 작업(최초 등록)한 ASSY 품번**. 어디 쓰이든 이 코드 그대로 재사용.
