...
### A1. ② 데이터 이관 완주(2501~ 전표·원장) + 일·월마감 — ✅**재확인 완료(2026-09-09)**
- 이관 자체 정상(9/7 실측 diff0: 재고 2,565품목·실적 115,600행).
- ✅**월마감: MAT·PRD·SAL 전 도메인 2601~2608 전부 close_flag=1**(period_close 실측·미마감 0). 마감 재설계 PR#197 완비.
- ⚠**일마감 mat_stock_daily = 260906(3일 lag)**: 매일마이그에 **일마감 단계 없음**(delta_sync만)→별도 프로세스. **컷오버 직전 1회 실행 필요**(A4 게이트 편입). 롤포워드가 gap 커버(A3).
