...
- ★**대표 결정(2026-09-09): 7~9월 이관·마감 완주는 컷오버 시점에 정리**(레거시 정지·데이터 멈춘 후). 지금은 데이터가 움직여 무접촉. ⟹ A1/A3는 **컷오버 절차(A4)에 편입**, 사전작업 아님.
...
- ★**대표 결정(2026-09-09): 음수 정리는 컷오버 시점**(7~9월 이관완주와 한 흐름·데이터 정지 후). 지금 무접촉. A4에 편입.

### A4. 컷오버 당일 순서 + 재검증 게이트 (데이터 정지 후)
★A1/A3(7~9월 이관·마감·음수정리)를 여기서 정리(대표 결정 2026-09-09·데이터 움직여 사전 무접촉).
1. **레거시 차단**: ilshin 권한회수 → 데이터 이동 정지.
2. **delta_sync 정지** + cutover_mark --set(do_full 자가거부·웹행 보호).
3. **7~9월(2507~) 이관·일월마감 완주**: 6월말 기초 위 7~9월 전표(특히 입고) 완전 이관 → 마감 엔진 2507~2509 실행. = 요구② 완주.
4. **음수재고 0**: 완주 후 잔존 음수 정리(B-2 4-a·소급 아닌 시점정리) → 화면 음수 0 확인.
5. **nx vs ORG diff0**(재고·실적 전 재고점) + 화면 음수 0.
6. **미러 직독 0**(동결 stale될 것): A2 잔여 전환 — item.py ITEM_SUB read+write→미러(이 시점) · 라인달력 A/B 확정.
7. **생산계획 diff0(일순위)** + 편성 소요 diff0(USE_QTY_PR) 재확인.
8. **flip**: `_conn` dbo→nx + 미지정 참조 nx 명시(31곳·2026-09-07 장애 재발방지).
9. **롤백 대비 --snapshot** 1회(cutover_rollback.py).
