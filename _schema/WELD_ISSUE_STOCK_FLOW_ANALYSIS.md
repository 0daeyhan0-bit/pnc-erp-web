...
## 13. ★e2e 검증 (2026-08-27) — 실제 바코드 생산실적

실제 바코드 GP02637562(품목 AJR30027705·공정 S4·용접봉 RAC30599301, 투입공정 Q1000) 로 procbc/save(8011) 호출:
- **★생산실적 잘 잡힘 + 용접봉 잘 떨어짐**: post +1 → ok:True, `stock.weld={consumed:0.015,kinds:1}`, 생산창고(Q1000) 재고 1.015→1.0 (−0.015 차감). 실적net=1.
- **취소 복원**: 취소경로(qty<0)는 ⑦ 자재게이트 스킵→⑨ 도달, 용접봉 +복원(net0).
- **★발견: ⑦ 자재게이트가 ⑨보다 먼저 걸림**. AJR30027705는 SUB(AJR30027705-SUB1 등)가 파트창고 재고0이라 ⑦에서 막힘 → 용접봉 게이트는 자재게이트 통과 후 작동. (둘 다 정상 보호막)
- **데이터 원상복구·무드리프트**: 테스트로 손상된 실적 완전복구(STICKER 원본1행), stock_ledger WELDTEST 0, 파트창고 재고 드리프트 없음(재고 net-0 정리), 레거시 net-0 잔재 14행 정리.
- ★배포전제 재확인: Q1000 생산창고 용접봉 기초재고 시드 필요(안하면 전 용접봉생산 차단).

## 14. 용접봉 단순화 착수 — 오분류 정리 (2026-08-28)

생산 요구: 용접봉을 **%등급×형태(봉/와이어) 유형**으로 단순화. 자재/마감은 구체품목(벤더별) 유지 = 2계층. 가격=%별 가중평균(형태·벤더 편차는 흡수). ★소요=중량(kg): base 용접봉 단위=KG.

### 14-1. ★fix#1 실리콘/구리스 오소비 (완료)
- **버그**: RAC31704701/702(Silicon)·RAC36134101(진공구리스)가 `nx.bom.role='용접봉'`+RAC prefix → backflush가 용접봉으로 −W 오소비. (실증: AAA30136702 용접봉수집=[RAC31704701] 실리콘뿐)
- **fix**: role '용접봉'→'매입기타' (19 edge, 백업 nx.bom_bak_silrole). role은 원가·소요 무관(nx_cost_engine·soyo 미사용) → **cost diff0 불변·검증**. backflush 실리콘 미소비 확인.
- 마스터는 이미 정확(sgroup 230/992·make3·cost_gubun2·EA) — BOM role만 오분류였음.

### 14-2. 남은 오분류/미완 (다음 fix)
- ★**RAC30599327-1(AG3%WIRE)·RAC30599328(30%원봉)**: 진짜 용접봉인데 sgroup 910(240 미이동) → 240 이동 필요.
- **땜납(SOLDER) 6종**: RAC30598002·30599310·30599334·32977302·35117401·35117409 — 용접봉인지 별범주(땜납)인지 자재/생산 확정 필요. (일부는 %변형 있어 실제 용접봉일 수 있음: RAC30599301 base명 SOLDER이나 변형 "1% 각봉")
- 이름공백 RAC 13종·단위 EA혼재·가중평균 단가 운영방식(갱신주기·저장위치).

### 14-3. fix#2 미이동 용접봉 240 이동 (완료 2026-08-28)
- RAC30599327-1(AG3%WIRE·62edge)·RAC30599328(30%원봉·3edge): sgroup 910→240 (백업 nx.item_bak_weld910).
- material_split 격리검증(AJR74302405-은납): 변경전=후 {won570·bu532.92·sa0} 동일 → **원가 diff0**(910·240 모두 bu버킷).
- sgroup=240 총 66개. **남은 910 용접봉 0 = 모든 진짜 용접봉(KG·브레이징)이 240으로 정합.**
- 향후 매일 sync 보존: r_item_sync가 sgroup 제외라 유지됨(기검증).
