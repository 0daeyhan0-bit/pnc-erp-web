...
7. **기초 스냅샷 심기** — ※2026-09-01 에 "컷오버 차단 사유" 로 승격했다가 **같은 날 정정**(근거 약함).
   재생에서 생산실적이 **전량 자재부족으로 거부**됐다. 원인 = **준비재고 기초 미적재**.
   게이트의 RDY 축은 `nx.stock_ledger` 가 **유일 소스**인데(자재는 미러 폴백이 있어 값이 나온다)
   실측 우리 원장 RDY 19행/308 vs 레거시 `PU_T_READY_STOCK_MAINT` 942,153행.
   ⟹ 재생 FAIL 의 원인은 기초가 아니었다(레거시도 준비재고 0). 정정 = `REPLAY_PILOT.md §8`.
   ★단 심기 전 주의: 레거시 `PU_T_READY_STOCK` 합계가 **−124,986**(음수 다수)이라 그대로 심으면 안 된다.
     §B-2 4-a(마이너스 재고 정리)가 선행돼야 한다.
7. **기초 스냅샷 심기** — 각 재고점 기초(자재 2607=7월기말 등 확정 마감스냅샷)를 nx.stock_close_snap에 적재. ★월표기=그 달 기말(2607=7월기말=8월기초).

### B-3. BOM·계획 정합
8. ~~BOM except_flag 재싱크~~ — ✅ **2026-08-29 검증: 드리프트 0, 재싱크 불필요**.
   공통 키 37,281개에서 **플래그 불일치 0건**. 총수 차이(nx 2,062 vs 레거시 6,984)는
   레거시에만 있는 키 5,176개(그중 except=1 이 5,154, 자식이 용접봉 RAC* 가 **98.9%**) 때문이고,
   용접봉은 nx.proc_weld 로 분리한 **설계**다(DO_NOT_USE §10). 항목 종료.
9. ~~우리 편성 nx.plan_* diff0~~ — ✅ **2026-08-29 종료**. 계획당김은 한대윤 차장이 이미 해결·검증했다
   (`cc6f91d` 당김 후 소요일시 전달 · `4f0da52` 당일 클램프 → part_plan_ymd 81.56%→99.27%,
    **기준일 동일 행만 100.00%** 89,414/89,414).
   내가 본 −3,278행·+1일 952건은 **당김 결함이 아니라 기준일 불일치**였다
   (nx 260828 편성 vs 레거시 260827 편성). 기준일 하루 차이로 80%/100%/77% 로 출렁인다(`4f0da52` 실측).
   ★**계획은 매일 재편성되므로 반드시 같은 기준일끼리 비교할 것.** 컷오버 당일 recon 도 동일.
   상세 = `CUTOVER_CHECKLIST.md` "9번 결론".
   기존 기록 갭 ≈24,865 → **지금 3,278**(같은 기간 260828~ 기준, nx 79,849 vs 레거시 83,127).
   분해: 날짜만 하루 다름 1,251+1,207 · **용접봉(240·910) 1,377 = 8번과 같은 설계** ·
   그 외 ~1,600 · 수량 다른 키 410(**0.55%**).
   날짜 빼고 보면 공통 74,496 = 대부분 **같은 소요를 다른 날짜에** 잡은 것.
   ☐ 남은 확인 3가지(날짜 하루차 원인 · 그 외 1,600 · 수량 410) — 컷오버 차단 사유는 아님.
   상세 = `CUTOVER_CHECKLIST.md` "8·9번 검증".
...
16. **롤백 계획** — ✅ **2026-08-29 수립**. 도구 `_migration/cutover_rollback.py`.
    ★"코드만 되돌리면 된다" 는 틀리다 — 컷오버 순간부터 웹 입력이 nx 에만 쌓이고 레거시엔 없다.
      롤백 첫 질문 = **"지금 되돌리면 몇 건이 사라지는가"**(`--diff`). 그 수를 모르면 결정 불가.
    ★컷오버 **직전에 `--snapshot` 을 반드시 1회** 돌릴 것(안 남기면 판단 자체가 불가능).
    ★코드 되돌림은 `git revert` → `main` push → `deploy_pull.ps1` 이다.
      운영폴더는 `--ff-only` 라 되감기가 안 된다. **운영에서 `git reset` 금지**(다음 배포가 죽는다).
