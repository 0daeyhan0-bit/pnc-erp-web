...
| 2026-08-23 | **중량 walker**(weight_explode=weight_calc._explode 재현) | v_cs_bom 부모 표본 450(전수 census 진행) | **★DIFF0 PASS 450/450**(raw_kg) | 같은 BOM 다른 필터: 원가=cs_calc / 중량=SAGUB(sagub_default=1 제외). ★소스 등가 선검증(nx.bom_line≡v_cs_bom 멤버·qty·sagub 0차). RAC 포함(폴백조건 raw==0 AND weld==0 보존). COOP_SET(coop_raw_spec)리프+coop_bom폴백. ★leaf 소스=**nx.PR_M_ITEM**(nx.item은 net_weight=geom·length 드리프트 3H00627M 0.3332→0.2907·5210A30999H len184vs188 → PR_M_ITEM 직독으로 diff0). |
| 2026-08-23 | **☐추후과제: 견적기준 중량 로직 검증**(사용자 지적) | — | 기록만(미착수) | 자재매출마감(salemagam) 실제 원소재/용접봉 **수불정산이 쓰는 중량 = 협력사 견적기준**(weight_calc.compute_quote, coop 협의중량 unit_wt)로 **walker의 geom/BOM기준(weight_explode=_explode)과 다름**. weight_explode는 _explode(geom BOM)를 diff0 재현했을 뿐. **① compute_quote(견적기준) 로직 자체가 맞는지 ② 통일엔진에 견적기준 중량 walker로 별도 편입할지** 추후 검토. 용접봉 소요 정본=CS_T_ITEM_WELD×1.5(_load_weld), proc_weld는 사내 재고차감 전용([[newerp-weld-settlement-roadmap]]). |
