...
### 1-2. 모드 walker (각각 얇은 순회)
| 모드 | 정지 규칙 | 필터 | 집계/값 | 현행 대응 |
|---|---|---|---|---|
| **원가 재료비** | INNER_PROD=0(매입/외주완성)·cg5(직납)에서 정지 | cs_calc_except=1 스킵 | leaf 값(소재단가×중량 or 매입가)+LME | nx_cost_engine material() |
| **생산 소요** | 사급(PART_DTL 가공공정)·최하위 | except_flag=1 스킵, 910용접봉 제외 | 최하위집계 SUM(×cum_use) | soyo STEP5~7 |
| **중량 정산** | INNER_PROD=0 정지 | cs_calc_except=1·**sagub=1 leaf** 제외 | leaf 중량 합 | weight_calc _explode |
...
- **정지 규칙이 모드마다 다른 게 핵심** — 원가=INNER경계, 생산=사급경계. 통일엔진은 **전개는 full, 정지는 walker 파라미터**로.
- 용접봉: 원가·중량은 공정종속 별도값(is_weld 분기), 생산소요는 제외(910).
