...
☐ **대표 확인 필요**:
1. 지금 수입검사를 **실제로 하는가**? (시스템 기록은 4년 넘게 없다)
2. 한다면 **어디에 기록**하는가?
3. 앞으로 한다면 **검사 대상 판별 기준**은? (품목별 / 협력사별 / 건별)

⟹ 답에 따라 갈린다:
- **안 한다** → IQC 분기를 빼고 단순화(입고 = 즉시 완료 90)
- **한다** → `insp_flag` 를 채우는 규칙 + 40(검사중)·승인 API 신설
