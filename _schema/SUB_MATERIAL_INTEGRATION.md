...
- **규칙**: 접미사 = 코드 첫 '-' 뒤 전부(예 `5210A13011H-15-1`→`15-1`·`-은납`→`은납`·`-S6-2`→`S6-2`). 품명 = **`[-{접미사}] {원품명}`**(★사용자 확정 2026-08-26 형식: 자도번 -접미사 모양 그대로 대괄호). **skip 4종**: ①접미사없음(clean코드) ②원품명에 코드포함(`4849A20069M-7-1`=품명) ③이미 `[-접미사] `로 시작(멱등) ④원품명이 접미사시작(`은납-SUB`).
- **★원품명(base)=라이브 PR_M_ITEM 직독**(무손상 원천) → 옛형식/재실행 자동교정(프리픽스 누적 없음·멱등). 대상=nx.PR_M_ITEM(displays 품명 원천).
- **실측·구현완료(2026-08-26·dev nx)**: `_migration/sub_norm/r_sub_desc_suffix.py`(DRY/`--commit`). sub_code_map raw_item 3,418 → **병기 1,975 commit** / skip=접미사없음1,227·코드포함131·접미사시작83·품명없음2. **재실행 변경0=멱등 확인.** 예 `Tube,Connector`→`[-15-1] Tube,Connector`·`사내 SUB`→`[-S6-2] 사내 SUB`.
- **★일 루틴 편입완료**: `CUTOVER_MUST_AND_DAILY_MIGRATION §A 2-b` — 매 sync 직후 `r_sub_desc_suffix.py --commit` 재실행(마스터 전체재복사가 덮으므로). ∴ nx 항상 유지·컷오버 부담0.
- **주의**: 공유 nx라 운영 화면 품명 즉시 반영됨(사용자 A안 승인). 품명 매칭은 item_code 기준이라 영향 적음.
