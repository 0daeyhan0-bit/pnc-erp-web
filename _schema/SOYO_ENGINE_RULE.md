...
| 5 | `setin.py:351`(_set_bom_expand)·`procbc.py:74`(_bc_bom) | 세트입고 명세/차감 | **`setin_soyo`(신규 walker)** | ✅**setin 완료(2026-09-08)**: 거래처-path walker `setin_soyo`(순환방지·INT누적·원자재정지·set_except) 신설, 옛 _DW6_SQL과 **diff0 62/62·엔진+cost 50/50**. ★부수: 앞선 세트CTE 앵커 nx.item 전환이 재귀CTE 타입불일치 유발→CAST(varchar50) 수정. procbc는 미착수 |
