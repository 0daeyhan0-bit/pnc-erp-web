...
| 7 | `gagong.py:214/545/622` | 가공진척 재고충당 롤업 | `stock_flow_rollup` | ✅**214·622 완료(2026-09-08)**: 재고충당 상향롤업 2곳 → `stock_flow_rollup`(214=4축 seed자재/파트/사급/생산·fixm **2325/2325 diff0**·음수재고 전파 포함 / 622=kitting 캐시 재사용+폴백 엔진화). ☐**545(CTE_BOM) 남음** = P2 멤버십 판정 top-down 전개(work_code 라우팅·charindex 경로중복제거·pr_m_mat 경계) — 전용 walker 필요(복잡·표시성). |
