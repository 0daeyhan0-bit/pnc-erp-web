...
## 6. ★★★컷오버 후 미러 직독 전수 스캔 & 전환계획 (2026-09-07)

> 컷오버(2026-09-07 ~20:12) 실행 후 대표 지시로 재감사. **핵심: FLIP은 `dbo.`→`nx.`(데이터베이스)만 바꿨지, 미러→클린을 바꾼 게 아니다.** 그래서 `nx.` 안의 **미러 테이블을 읽는 코드가 그대로 남았고**, 이제 레거시가 죽어 미러는 **동결**됐다. 편집은 클린에 쌓이는데 조회는 미러를 봐 **시간이 갈수록 드리프트**한다(§1-9-1 하드룰의 실체).

### 6-A. 미러 직독 전수 (SQL 문맥 FROM/JOIN/UPDATE… 기준, 주석 제외)
| 개념 | 미러 테이블 | 읽는 곳 | 클린 정본 | 편집 쓰기 위치(실측) | 위험 |
|---|---|---|---|---|---|
| 거래처 | `CM_M_CUST` | **146** | `nx.cust`(full)/`nx.partner`(slim) | **`nx.cust`**(/api/cust/save) | 🔴 활성 |
...
### 6-B. ★거래처 3-테이블 정정 (8/26 §쌍2 갱신)
8/26 감사는 `CM_M_CUST`(미러) vs `nx.partner`(클린) 2벌로만 봤으나, **실제 편집 대상은 제3의 `nx.cust`** 였다:
| 테이블 | 컬럼 | 행수 | 정체 |
|---|---|---|---|
| `nx.cust` | cust_code·cust_name·biz_no·owner_name·biz_type·addr… (full) | 357 | **거래처 편집 프로그램이 쓰는 실질 정본**(9/7 수정 1건) |
| `nx.partner` | partner_code·partner_name·partner_type·remark (4컬럼) | 357 | nx.cust의 slim 투영(코드 100% 일치) |
| `CM_M_CUST` | 12+컬럼(사업자·대표자·주민·주소…) | 361 | 미러(레거시 복사·동결) |

- **현재 드리프트 실측**: 이름 다름 **0** · nx.cust에만 0 · **미러에만 4코드**. → 아직 거의 안 벌어짐(컷오버 직후). 단 메커니즘 실재: 거래처 수정/신규가 `nx.cust`에만 쌓이면 146곳이 옛 이름·누락을 봄.
- **정본 방향**: 실질 정본 = `nx.cust`(full schema + 편집대상). 146곳 대부분은 표시용 거래처명 조인 → `nx.cust`로 전환 가능(사업자번호 등 full 컬럼도 nx.cust에 있어 커버). `nx.partner`(slim)는 이름만 필요한 곳의 축약뷰.
- 미러만 남은 4코드 = nx.cust 편입 검토(폐업/미사용 여부 확인 후).

### 6-C. 전환계획 (옆에짓고 diff0·§1-9 원칙)
공통 원칙: **엔드포인트별 before/after 결과 diff0 확인 후 교체**(값 바뀌면 왜 바뀌는지 실측·기록). 미러→클린은 값이 달라질 수 있음(신규/재분류 반영).
1. **단가 7곳**(PR_M_ITEM_COST→nx.price_item) — 수량 적고 편집 활성. **1순위**. 단 sourcing "정산마스터=불변조회" 규약(§쌍5) 위반 여부 개별 확인.
...
---
*이 감사는 살아있는 문서. 수렴/은퇴가 진행되면 갱신. 코드매핑 원본 = Explore 스캔(2026-08-26) + 컷오버후 재스캔(2026-09-07).*
