...
## ★5-A. 7~9월 이관·일월마감 완주 + 음수 정리 (요구②③·2026-09-09 대표 결정) — ★핵심

**왜 여기서**: 이관·마감이 **올해 1~6월(2501~2506)만** 됐다(mat_stock_daily 기초=6월말). 7~9월은 불완전 rollforward라 **자재 음수 139품목 −39,597**. 데이터가 움직이는 병행운영 중엔 손대지 않고, **레거시 정지(4·5번) 후 데이터가 멈춘 이 시점에** 완주·정리한다(매일마이그 타이밍 원칙과 동일).

1. **7~9월(2507~) 전표·원장 완전 이관** — 특히 입고전표(67품목 이관누락·66품목 입고이관누락 의심). 6월말 기초 위에 얹는다.
2. **마감 엔진 2507~2509 실행**: `/api/close/run`(MAT/PRD/SAL · D/M) → nx.stock_snapshot+period_close. (설계 create_period_close.py)
3. **음수재고 0 확인 + 잔존 정리**(B-2 4-a·시점정리·소급 아님). 화면 음수 0.
근거·경위 = `CUTOVER_MUSTDO_260909.md` A1·A3·A4 · `CUTOVER_RETRY_REQUIREMENTS_260907.md` ②③.
