...
>
> **★★★`PARTNER_ERP_TEST3.dbo.*` 사본은 절대 읽지 말 것 — 전부 낡았다(2026-09-08 실측·대표 지시).**
...
> | PU_T_READY_STOCK_MAINT | 949,698 | 902,236 | **260907** | 260716 |
> | PR_T_PLAN_PART_MAT | 117,824 | 67,125 | — | **dbo 43% 결손** |
> | PR_T_PLAN_DTL | 4,771 | 3,248 | — | **dbo 32% 결손** |
>
> **위험한 이유 = 실수하기 쉽다.** 컷오버로 `_conn()`·`_ro()` 의 기본 DB 가 `PARTNER_ERP_TEST3` 가 됐으므로,
...
> · 잔존 지뢰 `live_api.py:140` `_LEDGER_SELECT` 의 `from {tbl} t` — L160·171 이 무수식으로 넣는다.
>   같은 함수 L157·168 은 3-part → **키는 nx 에서, 데이터는 dbo 에서** 읽는 상태.
> ⟹ **모든 SQL 은 `PARTNER_ERP_TEST3.nx.<테이블>` 3-part 로 쓴다.**
>   특히 `.format()`·f-string 으로 **테이블명을 조립**하는 곳 주의(컷오버 치환 스크립트가 이걸 못 잡았다).
> ⟹ **dbo 사본 삭제는 무수식 참조 전수 3-part 화가 끝난 뒤에만** 가능(지금 지우면 그 코드가 즉시 깨진다).
>
...
| 6b | **☐원가 오라클 게이트·전수 불일치(과제)** | **COST_ORACLE_GATE_AND_DIFF_BACKLOG.md**(08-31, 미착수 — anchors가 엔진 미대조·전수 12.6% 불일치) | LEGACY_BUG_CANDIDATES |
...
> ★★★**컷오버 문서 세트(2026-09-07~08)** — 재컷오버 작업 전 이 순서로 읽는다.
> | 문서 | 담당 | 축 |
> |---|---|---|
> | `CUTOVER_RETRY_REQUIREMENTS_260907.md` | **THE 원칙(단일 데이터셋)·재컷오버 순서 0~5** | **상위 정본** |
> | `MIRROR_CLEAN_DUAL_TABLE_AUDIT.md §6` | 미러 직독 194곳·전환계획(단가6·품목13·거래처147 완료) | **읽기** |
> | `CUTOVER_PROCESS_AND_SOURCE_MAP_260908.md` | **프로세스·소스 위치 상세**(재고 3층·엔드포인트 흐름·줄번호) | **쓰기** |
> | `CUTOVER_FIX_PLAN_BY_SCREEN_260908.md` | 화면별 처방 P0~P2·작업순서 | 처방 |
> | `CUTOVER_SCREEN_DATASOURCE_ANALYSIS_260907.md` | 실행검증 요약·병존쌍 실측 | 근거 |
> | `CUTOVER_EXECUTION_LOG_260907.md` | 실행·롤백 로그 | 이력 |
>
> **C26 (신규) — 쓰기 경로가 이력을 안 남긴다**(읽기 축 감사로는 안 잡히는 유형).
> ⟹ **§6 감사는 "어디서 읽는가" 축이라 이 유형을 못 잡는다.** 검증 = `_migration/flow_all_testbed.py`(롤백보장·DB변경0).
>
> **C26-1 준비재고 이력 — ✅해소(2026-09-08)**. 정본 = **`nx.PU_T_READY_STOCK_MAINT`**(949,698행).
> **태그가 곧 화면이다**(실측 INSERT_WINDOW):
>
> | tag | 화면 | 부호 | 웹 라우터 |
> |---|---|---|---|
> | **1** | `w_pr_input_460_new` 준비실적 **등록** | + | `ready.py:515` **← 2026-09-08 신규** |
> | **2** | 〃 준비실적 **취소** | − | 〃 (`mode='cancel'`) |
> | A | `w_pr_input_260` 드래그실적 314,224 | − | `dragprod.py:315·540`(취소 대칭) |
> | A | `w_pr_input_520` 바코드실적 188,266 | − | `prodsheet.py:1592` |
> | B | `w_pu_ready_stock_010` 강제수정 | ± | (웹 미구현) |
>
> 소진(A)만 웹이 쓰고 **등록(1)·취소(2)를 안 써서** 이력이 계속 마이너스로 벌어졌다
> (실측 잔액 −152,328 vs 이력 −1,612,929 · 일치율 51.6%). 롤백검증: 등록+2 → 취소−2 **이력·잔액 모두 원복 ✅**.
> **이관 정상** — nx 949,698 vs 레거시 949,726(차 28 = 오늘 레거시 발생분) · **nx 에만 있는 행 0**.
>
> **C26-2 출하 클린 미기입 — 미해소**: `nx.sale_dtl`(클린 출하 정본)이 **260827 `migration` 기초이관에서 정지**.
> 출하실적등록은 미러 `SA_T_SALE_DTL` 에만 쓴다(실행검증 4사례). 읽는 곳은 이미 클린(`matinput.py:242`)
> → **010 화면은 8/27 이후 출하를 못 본다.**
>
> **C26-3 재고 표시축 불일치 — 미해소**(2026-09-08 실측):
> · **잔액이 실시간 현재재고 정본**(`PU_T_MAT_STOCK_WH`·`PR_T_MAT_STOCK_WH`) — 대표 확정.
> · 레거시 060 화면은 **당일=잔액 / 과거=이력합계** 로 분기하는데 **웹은 항상 이력누적**(`live_api.py:830`).
> · **`MAINT_QTY` 는 출고가 이미 음수로 저장**된다 — 부호를 또 뒤집으면 값이 망가진다(실측 오류 이력).
>   저장부호 그대로 + 직전월말 기초로 롤포워드 = **잔액과 96.1% 일치**(시료 10900O 차이 0).
> · **자재**: 2608 월스냅샷 **없음** → bf=0 → 9월 출고만 쌓여 화면이 음수(−184,724). **기초만 채우면 해결.**
> · **생산**: 스냅샷이 **2502 하나뿐**이고 `live_api.py:1332` 에 **하드코딩** → 3년 반치를 매번 재누적(느림·취약).
> · 준비등록·키팅 팝업은 **잔액 직독**인데 `ready.py:63` 주석만 "입출고현황과 동일 산식"이라 **stale**.
