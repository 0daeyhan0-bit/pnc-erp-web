...
## 10. ★★★조달경로 구분(제작/매입/사급) + 업체지정 재설계 (사용자 확정 2026-08-19)

**책임 분담**:
| 화면 | 결정 |
|---|---|
| **조달경로 통합검토**(개발·후보등록) | ①BOM 구조(SUB 그룹·계층·보관) ②**부품별 구분 제작/매입/사급** ③**SUB별 구분**(SUB 생성 시 매입/제작/사급 지정) |
| **조달 프로파일** | ④경로 활성·배분% ⑤**업체 지정**(매입/사급 부품별, ★현행 R01 발주업체·배분 모달과 동일 구조) |

**구분 의미**: 제작=우리가 만듦(내부·업체불필요) · 매입=외부구매(매입처필요) · 사급=유상사급(우리가 자재를 협력사 공급, 협력사 조립). SUB 단위 사급 지정 → 하위가 사급.

**업체 지정 = R01 구조 복제**: R01 발주업체·배분 모달(부품별 업체 autocomplete+배분%+[+업체추가]+마스터단가 읽기전용) → R02는 동일 UI, 저장만 sourcing_profile(경로별). 매입/사급 부품만(제작 제외). "개별 업체지정 힘든 구조" 해소.

**저장 게이트(구현됨)**: R02 활성인데 매입/사급 부품 업체 미지정 → 저장 차단(gate=VENDOR).

**★핵심 요구**: 이 구분(제작/매입/사급, SUB포함)이 **우리 생산계획(plan_part_mat/compose_mat) + 협력사 생산계획(coopplan)에 정확히 반영**되어야 함. **현재 반영되는지 검증 필수**(route line gubun → SUPPLY_GUBUN 흐름). ★현재 compose_mat은 대안경로 fallback에서 헤더구분 사용(§8 오류) → 라인구분 기반으로 교정 필요.
