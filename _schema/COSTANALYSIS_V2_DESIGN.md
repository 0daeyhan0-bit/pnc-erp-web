...
| 2026-08-22 | **STEP-U2 실행** (wc 흡수·routing_edge 은퇴) | soyo STEP7 item_ov=마스터 직독(routing_edge join 제거)+ /api/routing/sync=deprecated no-op. routing_edge 참조=soyo 단독(JS/SQL/PS1/마이그 전무). **검증**: ①item_ov ov_wc OLD(routing_edge) vs NEW(마스터) 불일치=0 ②plan_part_mat 재생성 diff0(진행중). 백업 nx.plan_part_mat_preU2_260822·routing_edge_bak_preU1. 근거=wc_user 편집0·wc≡마스터 42622/42625 | routing_edge=순수중복(편집0). 생산처 편집은 향후 조달프로파일 소유. src_type도 드롭(U1 마무리) |
