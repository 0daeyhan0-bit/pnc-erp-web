...
| 2026-08-22 | **STEP-U2 완료** (wc 흡수·routing_edge 은퇴) | soyo STEP7 item_ov=마스터 직독(routing_edge join 제거)+/api/routing/sync=deprecated no-op+_routing_edge_sync 제거. routing_edge→**RENAME `nx.routing_edge_retired_260822`**(즉시복구가능). 실제 plan_part_mat 미변경(98888 유지, 검증은 별도 test테이블로 비파괴). **검증 PASS**: ①item_ov ov_wc OLD vs NEW **불일치0** ②plan_part_mat **생산처(WC) old-logic vs new-logic 불일치0**(격리대조, 동일 upstream). 잔여 168=전부 BCUP1S(브레이징봉) **수량**차=ov_wc 수식무관→U2무관(빌드 시점차 v_pr_bom 드리프트/기존 비결정성). 배포코드 item_ov 24115품목 빌드OK. **결정성 재확증 테스트는 DB커넥션리셋으로 미완**(로직 아님)—수학+WC0로 결론 확정 | routing_edge=순수중복(wc_user편집0·wc≡마스터42622/42625). 생산처 편집은 향후 조달프로파일 소유. **협력사계획 diff0=구조상**(plan_part_mat 생산처 불변+plan_mat_source overlay=route_alloc/sourcing_profile 무변경). ★별건: BCUP1S 브레이징봉 수량 비결정성(기존·옆세션 소요도메인). dev만·미배포 |
...
### ★설치 음수델타 근본원인 = maint 데이터 오염 (2026-08-22, v2_seolchi 노드실측)
- **범인=단일 자재 `5210A22977Q`**(buy·EA): eng_mat 15,986 vs jik 3,462, **lo~hi 270~15,986(59배)**→delta −12,524. PQ061203K11(−16,085)·PQ061208C11(−12,787) 음수의 대부분이 이 한 노드.
- 270~15,986=가격변동 아님=**쓰레기 데이터**(동일코드 다spec/단위/오류 혼입). 가중평균 jik 무의미. 나머지 매입노드는 delta≈0.
- **∴ 규칙·방향 정상. 설치 음수=소수 자재코드 maint 오염**(spread>2 플래그가 정확히 포착).
- **★fallback 확정(§5F 원칙)**: **maint 스프레드 hi/lo>3 자재는 override 스킵→엔진값 유지**. 신뢰가능(스프레드 작은) override만 적용. 정밀화=median/trimmed avg·spec별 분리(추후).
- 부산물 재확인: 5210A22977R eng310 vs 실매입~1288(마스터 stale·저평가) — 단 이것도 스프레드 260~7031 커서 신뢰도 낮음. 마스터 매입단가 품질 이슈 별도 등록.

### ★가드 적용 결과 = 설치 음수 해소 검증 (2026-08-22, v2_c4)
- **spread>3 fallback 적용**: PQ061203K11 raw−16,085→**가드−50**, PQ061208C11 −12,787→−1,241. 오염 소멸.
- **방향 정상화**: PQ060903E30(설치 고강도) +2,668(실매입>사급가 상향, §5Q·사용자설명 일치). 사급앵커 AJR75563503 −4(diff0).
- **스코프25 델타: raw −35,873 → 가드 −12,485**(잡음 −23,388 제거). 제품별 혼재(+상향/−하향)=실제 실매입 vs 계상 차이의 정확 반영.
- **★Phase1 계산로직 검증완료**: ①직구매실매입맵(사급vendor제외) ②단위인식override(매입=jik×qty·원소재KG=jik×wt×qty·원소재EA=jik×qty) ③spread>3 fallback(엔진값) ④사급품 diff0. → 다음=래퍼모듈(nx_cost_v2)+/api/cost/nx_v2+SCREEN연결+전스코프(253) 검증.
- **별도 등록 이슈**: (a)오염 maint 자재(5210A22977Q 등 spread극단)=매입데이터 spec혼입 정리 대상 (b)매입 원소재 마스터단가 stale(5210A22977R). 둘 다 데이터품질(원가로직 아님).
...
## §7. 손익 큰요인 감사 + 단가완결성 (2026-08-22, 사용자 "레거시 손익 큰영향 요인 1차 검증")

### 전 스코프 V2 재료비 검증 완료
- 스코프 **263품번**(리시빙 상위25%·매출 95.4%커버, 2601~), 계산263·에러0. **사급품 186 diff0**·값변화 77.
- **수량가중 손익 impact = 순 +0.41억** — ★제품별로는 ±수천만원 대폭 스윙(상쇄): PQ060905R01 **+7,040만**(실매입<사급가)·PQ060903E30 **−5,719만**(실매입>사급가)·AJJ75838626 −2,280만. 순액작음≠영향작음(제품별 정확도는 큼).

### ★사용자 지목 큰요인 (우선순위, 미검증)
1. **사급가 vs 실매입가** = 위 검증(단일시점). 제품별 큼.
2. **★날짜 squeeze(R-7)**: 판가 지연(TAGE as-of) vs 실매입 즉시 → 시차 마진눌림. **월별 시계열 필요**(단일시점 스냅샷은 못봄). ★방법론: 누적 가중평균은 timing평탄화 → period 실매입가 필요.
3. **★회수율(PROD_RATE)**: 엔진·레거시SP **둘다 원가에 미사용**(grep0). 마스터 제품 prod_rate=100 대부분(평균99.3%)인데 **사용자 실제 회수율 55%** → 실제55%가 마스터에 없음+원가 미반영 = 원가 과소·손익 과대 가능(큰요인). ★정의 확인필요(원소재 절삭회수율/생산수율/ST효율 중 무엇, 어디 적용). 사용자 설명 대기.
4. **LME 사급차액** · **판가 as-of 정확도**.

### §7A. ★가공비 ST 회수율(효율) 반영 — 설계확정, 적용은 추후 (사용자 2026-08-22)
- **개념**: 엔진 가공비 = 표준 ST × 임율(효율 100% 가정). 실제 효율 **평균 55%** → 실 ST = 표준ST ÷ 0.55(=×1.82). V1은 효율손실 미반영 → 가공비 과소·손익 과대(잠재 최대 지렛대, 가공비가 실원가 상당부분).
- **★granularity(사용자 확정)**: **품목별 ST=미검증(부정확) / 라인별 ST=어느정도 정확** → 효율은 **라인 레벨 factor**로 적용(품목별 ST 정밀도 의존 금지).
- **설계(확정, 적용 추후)**:
  - **라인효율 테이블 `nx.line_efficiency` 신설**(nx 네이티브·마이그무영향): `line_code · efficiency_pct · apply_ym · note`. as-of(월별 시변). **비어있으면 전부 100%**.
  - **라인 매핑 = `nx.item.prod_line`**(실측 확인, work_code=P1/P2는 작업장이라 sparse). 품목→prod_line→효율.
  - **적용 공식**: V2 가공비 = Σ_라인( 엔진 가공비_그라인 × (100 / 효율_그라인@ym) ). 효율=100이면 ×1.0=엔진값. **품목별 ST 미검증→라인 집계에만 factor**(품목별 정밀도 의존 안 함).
  - **hook**: `line_efficiency(prod_line, ym)` → 효율%(default 100). wrapper가 가공비 노드를 prod_line별 합산 후 gross-up.
- **★일괄 금지(사용자 2026-08-22)**: 55%를 **전사 일괄 적용 금지**. 55%=전사평균일 뿐, 실제는 **라인별 효율** 각각. 라인효율 hook은 라인키로 조회(라인별 값), 없으면 1.0.
- **★적용 시점**: 사용자 "라인별 ST 회수율 적용은 **추후에** 할거야" → **지금은 설계·훅만**(효율=1.0으로 V2=V1 유지), 라인별 효율값 확보 후 실제 적용. [[newerp-prod-result-st]] ST_LIVE_FUNCTIONS 연계.

### ★단가 완결성 (사용자: "단가없는 품목 찾아서 넣는게 맞아", 올해1월~ 정확)
- **스코프 제품 중 LG판가 없음 11개**(AJR30133607/606·30125602·76402409 등) → 손익 계산불가.
...
## §11. ★논리검증: 판가·원가 기간매칭 필수 (2026-08-23, 이동평균 제안 검증)

사용자 제안: 실원가에 이동평균 가격. 검증 지시.

### 개념 논리 = 성립조건 3
①재고 commingled(사급+직구매)여야 이동평균=실제원가 ②생산시점≈출하시점 ③소요량 기준일치.

### ★실측 검증 (logic.txt, 판가=리시빙실적 기준)
- **AJR30027702: 재료비(사급가 211,633·이동평균 202,068) > 판가실적 192,984 → 손익 −34,888(적자!)**. 가공비 넣기도 전 적자=논리오류.
- AJR75563503 −1,870. → **판가(리시빙실적=기간가중=옛PO낮음) vs 원가(as-of 최신=높음) 기준 불일치→인위적 적자.**
- 이동평균 재료비 ≈ 사급가(약간 낮음: 27702 202,068 vs 211,633·30133601 187,255 vs 196,222). 둘다 as-of라 판가와 여전히 불일치.

### ★논리 판정
- 프레임(판가실적 − 이동평균원가) **원칙 정답**(실매출−실원가). **단 판가·원가 반드시 같은 기간(월별) 매칭**. 안하면 인위적 적자(검증됨). **사용자 "원가도 같은 기준" = 논리필수로 증명**.
- **올바른 구조**: `손익 = Σ_월[ 판가(그달 리시빙) − 원가(그달 이동평균) ] × 수량`. 양쪽 같은달 실제값. #3 월별접근과 일치.
