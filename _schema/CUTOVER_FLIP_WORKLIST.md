...
live_api.py     1225, 1234  (2·LG리시빙관리 lgrecv 조회 — 신규 리시빙파일 업로드와 짝)
                            ★2026-09-03 추가: 舊 미한정 `sa_t_lg_receiving_dtl` → `PARTNER_ERP.dbo.SA_T_LG_RECEIVING_DTL`
                            로 명시 한정 완료(오늘 동작 동일=라이브). 표준 flip(dbo→nx)이 그대로 잡음.
                            ※짝 프로그램: routers/lgrecv.py 업로드는 **항상 nx 쓰기**(flip 무관).
                              컷오버 후 lgrecv 조회=nx + 업로드=nx → 리시빙 단일소스 정합.
                              레거시 대체: w_sa_sale_110 "LG리시빙파일 업로드"(RPA→라이브) → 웹 업로드(→nx).
